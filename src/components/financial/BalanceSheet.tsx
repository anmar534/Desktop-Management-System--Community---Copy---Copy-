...
export const BalanceSheet: React.FC<BalanceSheetProps> = ({
    ...
    return [
      {
        ...
        items: [
          ...
          {
            label: 'أصول أخرى',
            value: assets.currentAssets.otherCurrentAssets + assets.nonCurrentAssets.otherNonCurrentAssets,
            percentage: ((assets.currentAssets.otherCurrentAssets + assets.nonCurrentAssets.otherNonCurrentAssets) / assets.totalAssets) * 100
          }
        ]
      },
      {
        ...
        items: [
          {
            ...
            percentage: liabilities.totalLiabilities > 0 ? (liabilities.currentLiabilities.accountsPayable / liabilities.totalLiabilities) * 100 : 0
          },
          {
            label: 'القروض قصيرة الأجل',
            value: liabilities.currentLiabilities.shortTermDebt,
            percentage: liabilities.totalLiabilities > 0 ? (liabilities.currentLiabilities.shortTermDebt / liabilities.totalLiabilities) * 100 : 0
          },
          {
            label: 'القروض طويلة الأجل',
            value: liabilities.nonCurrentLiabilities.longTermDebt,
            percentage: liabilities.totalLiabilities > 0 ? (liabilities.nonCurrentLiabilities.longTermDebt / liabilities.totalLiabilities) * 100 : 0
          },
          {
            label: 'المصروفات المستحقة',
            value: liabilities.currentLiabilities.accruedExpenses,
            percentage: liabilities.totalLiabilities > 0 ? (liabilities.currentLiabilities.accruedExpenses / liabilities.totalLiabilities) * 100 : 0
          },
          {
            label: 'خصوم أخرى',
            value: liabilities.currentLiabilities.otherCurrentLiabilities + liabilities.nonCurrentLiabilities.otherNonCurrentLiabilities,
            percentage: liabilities.totalLiabilities > 0 ? ((liabilities.currentLiabilities.otherCurrentLiabilities + liabilities.nonCurrentLiabilities.otherNonCurrentLiabilities) / liabilities.totalLiabilities) * 100 : 0
          }
        ]
      },
      {
        ...
        items: [
          {
            ...
            percentage: equity.totalEquity > 0 ? (equity.paidInCapital / equity.totalEquity) * 100 : 0
          },
          {
            label: 'الأرباح المحتجزة',
            value: equity.retainedEarnings,
            percentage: equity.totalEquity > 0 ? (equity.retainedEarnings / equity.totalEquity) * 100 : 0
          },
          {
            label: 'حقوق ملكية أخرى',
            value: equity.otherEquity,
            percentage: equity.totalEquity > 0 ? (equity.otherEquity / equity.totalEquity) * 100 : 0
          }
        ]
      }
    ]
  ...
  const financialRatios = useMemo(() => {
    ...
    // نسب السيولة
    const currentRatio = liabilities.currentLiabilities.totalCurrentLiabilities > 0
      ? assets.currentAssets.totalCurrentAssets / liabilities.currentLiabilities.totalCurrentLiabilities
      : 0

    const quickAssets = assets.currentAssets.totalCurrentAssets - assets.currentAssets.inventory
    const quickRatio = liabilities.currentLiabilities.totalCurrentLiabilities > 0
      ? quickAssets / liabilities.currentLiabilities.totalCurrentLiabilities
      : 0

    // نسب المديونية
    const debtToAssets = assets.totalAssets > 0
      ? (liabilities.totalLiabilities / assets.totalAssets) * 100
      : 0

    const debtToEquity = equity.totalEquity > 0
      ? (liabilities.totalLiabilities / equity.totalEquity) * 100
      : 0

    const equityRatio = assets.totalAssets > 0
      ? (equity.totalEquity / assets.totalAssets) * 100
      : 0
    ...
    return {
      ...
      leverage: [
        { label: 'نسبة الدين إلى الأصول', value: debtToAssets, benchmark: 50, isGood: debtToAssets <= 50, isPercentage: true },
        { label: 'نسبة الدين إلى حقوق الملكية', value: debtToEquity, benchmark: 100, isGood: debtToEquity <= 100, isPercentage: true },
        { label: 'نسبة حقوق الملكية', value: equityRatio, benchmark: 50, isGood: equityRatio >= 50, isPercentage: true }
      ]
    }
  }, [currentBalanceSheet])
  ...
  const isBalanced = useMemo(() => {
    ...
    const { assets, liabilities, equity } = currentBalanceSheet
    const difference = Math.abs(assets.totalAssets - (liabilities.totalLiabilities + equity.totalEquity))
    ...
    return (
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <Button onClick={() => {/* TODO: إضافة وظيفة إنشاء ميزانية جديدة */}}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {financialRatios.leverage.map((ratio, index) => (
                    <div key={index} className="p-4 border rounded-lg">
                      ...
                      <div className="text-2xl font-bold mb-1">
                        {ratio.value.toFixed(1)}{ratio.isPercentage ? '%' : ''}
                      </div>
                      <div className="text-sm text-gray-500">
                        المعيار: {ratio.benchmark}{ratio.isPercentage ? '%' : ''}
                      </div>
                    </div>
                  ))}
                </div>
