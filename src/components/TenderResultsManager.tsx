  ...
  const confirmMarkAsWon = async () => {
    ...
    try {
      ...
      if (projectCreationResult.success) {
        ...
        const errorMessage = projectCreationResult.errors?.join(', ') ?? 'لا يمكن إنشاء مشروع تلقائي';
        ...
        <div className="mt-4 p-3 bg-info/10 rounded-lg border border-info/20">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-info">
              ...
              {currentStatus === 'submitted' && (
                <p><strong>تم تقديم العرض بنجاح.</strong> بانتظار إعلان النتائج من العميل.</p>
              )}
              {currentStatus === 'won' && (
                <p><strong>🎉 مبروك! تم الفوز بهذه المنافسة.</strong> يمكنك الآن البدء في تنفيذ المشروع.</p>
              )}
              {currentStatus === 'lost' && (
                <p><strong>لم يتم الفوز بهذه المنافسة.</strong> يمكنك مراجعة التسعير للمنافسات المستقبلية.</p>
              )}
            </div>
          </div>
        </div>
