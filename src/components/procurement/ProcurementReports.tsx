...
export function ProcurementReports({ className }: ProcurementReportsProps) {
  ...
  // إنشاء تقرير جديد
  const generateReport = async (type: string, startDate: string, endDate: string, filters: any = {}) => {
    try {
      ...
      switch (type) {
        case 'purchase_orders':
          report = await procurementReportingService.generatePurchaseOrderReport(startDate, endDate, filters)
          break
        case 'supplier_performance':
          report = await procurementReportingService.generateSupplierPerformanceReport(startDate, endDate, filters.supplierIds)
          break
    ...
    <div className={`space-y-6 ${className}`} dir="rtl">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            جميع التقارير التي تم إنشاؤها مع إمكانية العرض والتصدير
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
...
function ReportCard({ report, onView, onExport, onDelete, getIcon, getStatusColor }: ReportCardProps) {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4 space-x-reverse">
          <div className={`p-2 rounded-lg ${getStatusColor(report.type).replace('bg-', 'bg-opacity-20 bg-')}`}>
            {getIcon(report.type)}
          </div>
          <div>
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          ...
          <DialogDescription>
            تم إنشاؤه في {formatDate(report.generatedAt)}
          </DialogDescription>
        </DialogHeader>
                    ...
                    <div key={performer.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-left">
                        <p className="font-bold">
                          {typeof performer.value === 'number' && performer.metric.includes('القيمة')
