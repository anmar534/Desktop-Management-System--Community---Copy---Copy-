...
export const InventoryManagement: React.FC = () => {
  ...
  const getStockLevel = (item: InventoryItem) => {
    if (item.maximumStock) {
      ...
      return item.currentStock > item.reorderPoint ? 100 : (item.currentStock / item.reorderPoint) * 100
    }
  }
                ...
                >
                  ...
                  <Badge variant={
                    ...
                    {alert.alertType === 'low_stock' ? 'مخزون منخفض' :
                     alert.alertType === 'out_of_stock' ? 'نفاد المخزون' :
                     alert.alertType === 'overstock' ? 'مخزون زائد' :
                            ...
                            <TableCell>
                              <Badge variant={item.status === 'active' ? 'default' : 'secondary'}>
                                {item.status === 'active' ? 'نشط' : 
                                 item.status === 'inactive' ? 'غير نشط' : 'متوقف'}
                              </Badge>
                            </TableCell>
