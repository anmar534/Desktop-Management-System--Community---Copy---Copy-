...
export const TenderPricingProcess: React.FC<TenderPricingProcessProps> = ({ tender, onBack }) => {
  ...
  const quantityItems: QuantityItem[] = useMemo(() => {
    ...
    if (quantityData.length === 0) {
      ...
      if (
        title.includes('مبنى') ||
        scopeText.includes('مبنى') ||
        title.includes('بناء') ||
        scopeText.includes('بناء')
      ) {
  ...
  const handleTemplateUpdate = useCallback(
    (template: {
      ...
      estimatedDuration: number
      defaultPercentages: { administrative: number; operational: number; profit: number }
      costBreakdown: { materials: number; labor: number; equipment: number; subcontractors: number }
      tags: string[]
    }) => {
      try {
  ...
  const handleTemplateDelete = useCallback((_templateId?: string) => {
    try {
      ...
      // templateId will be used when implementing actual deletion
    ...
    try {
      ...
      if (PRICING_FLAGS.USE_ENGINE_AUTHORING) {
        try {
          ...
          const toPricingResources = <
            TRow extends MaterialRow | LaborRow | EquipmentRow | SubcontractorRow,
          >(
            rows: TRow[],
          ): PricingResource[] =>
            rows.map(
          ...
          const rawEntries: [string, PricingItemInput][] = Array.from(pricingData.entries()).map(
            ([id, data]) => {
      ...
      const quantityTableWithPricing: PricingViewItem[] = (() => {
        if (
          PRICING_FLAGS.USE_DOMAIN_PRICING_UI &&
          domainPricing.enabled &&
          domainPricing.status === 'ready'
        ) {
  ...
  const calculateTotals = useCallback(() => {
    ...
    const subcontractorsTotal = currentPricing.subcontractors.reduce(
    ...
    const administrativeCost =
      (subtotal * (currentPricing.additionalPercentages?.administrative || 0)) / 100
    const operationalCost =
      (subtotal * (currentPricing.additionalPercentages?.operational || 0)) / 100
  ...
  const persistPricingAndBOQ = useCallback(
    async (map: Map<string, PricingData>) => {
      ...
      const isMeaningfulDescription = (value: string): boolean =>
        value.length > 0 &&
        !/^البند\s*\d+$/i.test(value) &&
        !/^بند\s*\d+$/i.test(value) &&
        !/غير\s*محدد/.test(value)
      ...
      try {
        const items = quantityItems
          .map<PersistedBOQItem | null>((quantityItem) => {
            ...
            const directUpstream = normalizeString(
              quantityItem.canonicalDescription ?? quantityItem.description,
            )
            const specBased = normalizeString(quantityItem.specifications)
            const fallback = quantityItem.itemNumber
              ? `البند ${quantityItem.itemNumber}`
              : `البند ${quantityItem.id}`
  ...
  useEffect(() => {
    ...
    if (defaultsChanged && pricingData.size > 0) {
      ...
      if (changedCount > 0) {
        ...
        console.info(
          `[PricingProcess] 🟢 Auto-propagated new default percentages to ${changedCount} items bound to old defaults.`,
        )
      } else {
        void persistPricingAndBOQ(pricingData)
        console.info(
          '[PricingProcess] ℹ️ Defaults changed but no items matched previous defaults (all customized).',
        )
      }
    } else {
      void persistPricingAndBOQ(pricingData)
  ...
  const calculateItemsTotal = useCallback(() => {
    ...
    quantityItems.forEach((item) => {
      ...
      const subtotal =
        itemTotals.materials + itemTotals.labor + itemTotals.equipment + itemTotals.subcontractors
      const adminPercentage =
        itemPricing.additionalPercentages?.administrative ?? defaultPercentages.administrative
      const operationalPercentage =
        itemPricing.additionalPercentages?.operational ?? defaultPercentages.operational
  ...
  const updateTenderStatus = useCallback(() => {
    ...
    const completionPercentage =
      quantityItems.length > 0 ? (completedCount / quantityItems.length) * 100 : 0
    const totalValue = calculateProjectTotal()

    const storedFiles = safeLocalStorage.getItem<StoredTechnicalFile[]>(
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try {
        ...
        console.warn('⚠️ autosave draft failed', e)
      }
    }, 1500)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        console.warn('⚠️ periodic draft save failed', e)
      }
    }, 45000)
  ...
  // إنشاء صف فارغ
  const createEmptyRow = <Section extends PricingSection>(
    type: Section,
  ): SectionRowMap[Section] => {
  ...
  const addRow = <Section extends PricingSection>(type: Section) => {
    console.log(`[TenderPricingProcess] إضافة صف جديد من نوع: ${type}`)
    ...
    setCurrentPricing((prev) => {
      ...
      const updatedPricing = mutateSectionRows(prev, type, (rows) => [
        ...rows,
        recalculateRow(type, newRow),
      ])

      console.log(
        `[TenderPricingProcess] تم إضافة صف جديد. عدد الصفوف الحالي: ${updatedPricing[type].length}`,
      )
            ...
            <CardContent className="p-3 pt-2">
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  <span>
                    تم إنجاز {completedCount} من {quantityItems.length} بند
                  </span>
                  ...
                  <tbody>
                    {quantityItems.map((item, index) => {
                      ...
                      const materialsTotal =
                        itemPricing?.materials?.reduce((sum, m) => sum + (m.total ?? 0), 0) ?? 0
                      const laborTotal =
                      ...
                      const subcontractorsTotal =
                        itemPricing?.subcontractors?.reduce((sum, s) => sum + (s.total ?? 0), 0) ??
                        0
                      const subtotal =
                                    ...
                                    <div>
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          ...
                                          <Badge
                                            variant="outline"
                                            className="text-info border-info/30 text-xs"
                                          >
                                            {formatCurrencyValue(
                                              itemPricing.materials.reduce(
                                                (sum, m) => sum + (m.total || 0),
                                                0,
                                              ),
                                              {
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          <div className="text-xs font-semibold text-success">
                                            العمالة ({itemPricing.labor.length} نوع)
                                          </div>
                                          <Badge
                                            variant="outline"
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          <div className="text-xs font-semibold text-accent">
                                            المعدات ({itemPricing.equipment.length} معدة)
                                          </div>
                                          <Badge
                                            variant="outline"
                                      ...
                                      >
                                        <div className="flex items-center gap-2">
                                          <div className="text-xs font-semibold text-secondary">
                                            مقاولو الباطن ({itemPricing.subcontractors.length}{' '}
                                            مقاول)
                                          </div>
                                          <Badge
                                            variant="outline"
                                            className="text-secondary border-secondary/30 text-xs"
                                          >
                                            {formatCurrencyValue(
              ...
              <CardTitle className="flex items-center justify-between text-base" dir="rtl">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      setCurrentItemIndex(Math.min(quantityItems.length - 1, currentItemIndex + 1))
                    }
            ...
            <CardContent className="p-3">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3" dir="rtl">
                <div className="md:col-span-2">
                  ...
                  <p className="text-sm font-medium text-foreground text-right line-clamp-2">
              ...
              <div className="mt-2">
                ...
                <p className="text-xs text-muted-foreground text-right leading-relaxed p-2 bg-muted/20 rounded border border-border">
                          ...
                          <tr key={material.id}>
                            <td className="border p-2">
                              <Input
                                value={material.name ?? ''}
                                onChange={(e) =>
                                  updateRow('materials', material.id, 'name', e.target.value)
                                }
                            ...
                            <td className="border p-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateRow('materials', material.id, 'description', e.target.value)
                                }
                                placeholder="الوصف"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2">
                              <Input
                                value={material.unit ?? ''}
                                onChange={(e) =>
                                  updateRow('materials', material.id, 'unit', e.target.value)
                                }
                                placeholder="الوحدة"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2 text-center">
                              <Checkbox
                                ...
                                onCheckedChange={(checked: boolean) =>
                                  updateRow('materials', material.id, 'hasWaste', checked)
                                }
                              />
                            </td>
                            <td className="border p-2">
                            ...
                            <td className="border p-2 text-center font-bold">
                              {formatCurrencyValue(material.total, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </td>
                          ...
                          <tr key={labor.id}>
                            <td className="border p-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateRow('labor', labor.id, 'description', e.target.value)
                                }
                            ...
                            <td className="border p-2">
                              <Input
                                value={labor.unit}
                                onChange={(e) =>
                                  updateRow('labor', labor.id, 'unit', e.target.value)
                                }
                                placeholder="الوحدة"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateRow('labor', labor.id, 'quantity', Number(e.target.value))
                                }
                                placeholder="الكمية"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2">
                              <Input
                                ...
                                value={labor.price}
                                onChange={(e) =>
                                  updateRow('labor', labor.id, 'price', Number(e.target.value))
                                }
                                placeholder="السعر"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2 text-center font-bold">
                              {formatCurrencyValue(labor.total, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </td>
                          ...
                          <tr key={equipment.id}>
                            ...
                            <td className="border p-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateRow('equipment', equipment.id, 'unit', e.target.value)
                                }
                                placeholder="الوحدة"
                                className="h-8 py-1 text-sm"
                              />
                            ...
                            <td className="border p-2 text-center font-bold">
                              {formatCurrencyValue(equipment.total, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </td>
                      ...
                      <tbody>
                        {currentPricing.subcontractors.map((subcontractor) => (
                          <tr key={subcontractor.id}>
                            ...
                            <td className="border p-2 text-center font-bold">
                              {formatCurrencyValue(subcontractor.total, {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                              })}
                            </td>
          ...
          <Card>
            ...
            <CardContent className="p-3">
              <Textarea
                ...
                onChange={(e) =>
                  setCurrentPricing((prev) => {
                    const next = { ...prev, technicalNotes: e.target.value }
                    markDirty()
                    return next
                  })
                }
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto" dir="rtl">
      ...
      <Tabs value={currentView} onValueChange={handleViewChange} className="mb-6" dir="rtl">
        ...
        <TabsContent value="technical">{renderTechnical()}</TabsContent>
      </Tabs>
      {leaveConfirmationDialog}
