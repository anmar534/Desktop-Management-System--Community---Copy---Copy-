...
export function AnalyticsOverview({ onNavigate, className = '' }: AnalyticsOverviewProps) {
  ...
  const loadOverviewData = useCallback(async () => {
    ...
    try {
      ...
      const winRate = bidPerformances.length > 0 
        ? (bidPerformances.filter(p => p.result === 'won').length / bidPerformances.length) * 100
        : 0

      const totalValue = bidPerformances.reduce((sum, p) => sum + (p.bidAmount || 0), 0)
      const avgMargin = bidPerformances.length > 0
        ? bidPerformances.reduce((sum, p) => sum + (p.profitMargin || 0), 0) / bidPerformances.length
        : 0
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <Badge className={getCategoryColor(card.category)} variant="outline">
                            {card.category === 'analytics' ? 'تحليلات' :
                             card.category === 'competitive' ? 'تنافسي' : 'ذكاء'}
                          </Badge>
                        </div>
                      </div>
