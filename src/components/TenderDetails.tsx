...
export function TenderDetails({ tender, onBack }: { tender: any; onBack: () => void }) {
  ...
  const [collapsedSections, setCollapsedSections] = useState<{[key: string]: {
    materials: boolean;
    labor: boolean;
    equipment: boolean;
  ...
  const toggleCollapse = (itemId: string, section: 'materials' | 'labor' | 'equipment' | 'subcontractors') => {
  ...
  const formatQuantity = useCallback((value: number | null | undefined, options?: Intl.NumberFormatOptions) => {
  ...
  const isPricingCompleted = (localTender?.pricedItems || 0) > 0 && (localTender?.totalItems || 0) > 0 && (localTender?.pricedItems || 0) >= (localTender?.totalItems || 0)
  const isTechnicalFilesUploaded = !!localTender?.technicalFilesUploaded || FileUploadService.getFilesByTender(tender.id).length > 0
  ...
  const finalQuantityData = unified.items || []
    const hasPricingData = finalQuantityData.some((it: any) => typeof it.unitPrice === 'number' || typeof it.totalPrice === 'number')
  ...
  console.log('[TenderDetails] Unified hook source', unified.source, 'items', finalQuantityData.length)

      return (
        <div className="space-y-4" dir="rtl">
          {unified.status === 'loading' && (
            <div className="p-4 rounded-md bg-muted/40 text-sm text-muted-foreground border">جاري تحميل بيانات التسعير...</div>
        )}
            ...
            <tbody>
              ...
              {(() => {
                ...
                return finalQuantityData.map((item: any, index: number) => {
                  ...
                  const renderKey = occurrence === 1 ? baseKey : `${baseKey}__dup${occurrence - 1}`;
                ...
                // تمت إزالة حقن البيانات التجريبية للحفاظ على البيانات الحقيقية فقط
                
                // إعادة التحقق من وجود بيانات التحليل بعد إضافة البيانات التجريبية
                ...
                console.log(`Item ${item.id || index}: finalHasBreakdownData = ${finalHasBreakdownData}`, {
                  ...
                  if (occurrence > 1 && occurrence === 2) {
                    console.warn('[TenderDetails] Duplicate item id detected, generating unique key:', {
                      id: baseKey,
                      renderKey,
                      occurrence
                    });
                      ...
                      <td className="border border-border p-2 text-right">
                        <div>
                          ...
                          <div className="font-medium text-sm whitespace-pre-line leading-relaxed">
                            {item.canonicalDescription || item.description || item.desc || item.name || item.title || item.itemName || item.specifications || 'غير محدد'}
                          </div>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            try {
                              ...
                              const evt = new CustomEvent('openPricingForTender', { detail: { tenderId: tender.id, itemId: item.id } })
                                    ...
                                    <table className="w-full text-xs bg-card">
                                      ...
                                      <thead>
                                        <tr className="text-foreground bg-info/20 border-b-2 border-info/30">
                                          ...
                                          <th className="text-center p-2 font-semibold">سعر الوحدة (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">الإجمالي (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">ملاحظات</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                                    ...
                                    <table className="w-full text-xs bg-card">
                                      ...
                                      <thead>
                                        <tr className="text-foreground bg-success/20 border-b-2 border-success/30">
                                          ...
                                          <th className="text-center p-2 font-semibold">معدل الأجر (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">الإجمالي (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">ملاحظات</th>
                                        </tr>
                                      </thead>
                                    ...
                                    <table className="w-full text-xs bg-card">
                                      ...
                                      <thead>
                                        <tr className="text-foreground bg-warning/20 border-b-2 border-warning/30">
                                          ...
                                          <th className="text-center p-2 font-semibold">الوحدة</th>
                                          <th className="text-center p-2 font-semibold">عدد الساعات/الأيام</th>
                                          <th className="text-center p-2 font-semibold">تكلفة الساعة (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">الإجمالي (ر.س)</th>
                                          <th className="text-center p-2 font-semibold">ملاحظات</th>
                                        </tr>
                                      </thead>
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'pricing_in_progress': return 'جاري التسعير'
      case 'pricing_completed': return 'تم التسعير'
      case 'submitted': return 'بانتظار النتائج'
      case 'won': return 'فائزة'
      case 'lost': return 'خاسرة'
      case 'expired': return 'منتهية'
      case 'cancelled': return 'تم إلغاؤها'
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {tender.status === 'submitted' && (
              <TenderStatusManager tender={tender} />
            )}
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium">{tender.client || tender.ownerEntity || 'غير محدد'}</div>
                  </div>
                  <div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{tender.executionPeriod || tender.projectDuration || '-'}</div>
                  </div>
                  <div>
                    ...
                    <div className="font-medium">
                      {(() => {
                        ...
                        const bookletPrice = (tender as any).bookletPrice;
                        ...
                        const numeric = typeof rawPrice === 'string'
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{tender.category || tender.classification || '-'}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium">{tender.performanceBond ? `${tender.performanceBond}%` : '-'}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
