...
import { PageLayout, DetailStats, EmptyState } from './PageLayout'
...
const FINANCIAL_TABS = ['overview', 'projects', 'tenders', 'invoices', 'budgets', 'bank-accounts', 'reports', 'cost-analysis'] as const
...
const FINANCIAL_TAB_CONFIG: Record<FinancialTabValue, { label: string; shortLabel: string; icon: LucideIcon }> = {
  overview: {
    label: 'نظرة عامة',
    shortLabel: 'عامة',
    icon: BarChart3,
...
export function Financial({ onSectionChange, initialTab }: FinancialProps) {
  ...
  const formatSignedInteger = useCallback(
    (value: number) => {
      const normalized = Number.isFinite(value) ? value : 0
      const formatted = formatIntegerValue(Math.abs(normalized))
  ...
  const activateTab = useCallback((value: FinancialTabValue) => {
    setActiveTab(value)
  ...
  >(() => {
    if (budgetsAtRisk.length > 0) {
      ...
      const criticalBudget = budgetsAtRisk.some((budget) => Number(budget.utilizationPercentage ?? 0) >= 110)
      return {
    ...
    if (topOutstandingInvoices.length > 0) {
      const overdueCount = topOutstandingInvoices.filter((invoice) => invoice.status === 'overdue').length
      const totalOutstanding = topOutstandingInvoices.reduce((sum, invoice) => sum + Number(invoice.total ?? 0), 0)
      return {
    ...
    return null
  }, [budgetsAtRisk, formatCurrencyValue, projectsAtRisk.length, tendersClosingSoon.length, topOutstandingInvoices, formatIntegerValue])
    ...
    return [
      {
        ...
        trend: 'up' as const,
        trendValue: formatSignedPercentage(financialData.revenue.growth, 1),
        color: 'text-success',
      ...
      },
      {
        ...
        trend: 'up' as const,
        trendValue: formatSignedPercentage(8.2, 1),
        color: 'text-primary',
      ...
      },
      {
        ...
        trend: 'up' as const,
        trendValue: formatSignedPercentage(15.3, 1),
      ...
      },
      {
        ...
        trendValue: metrics.summary.outstandingInvoices > 0
          ? formatSignedPercentage(-3.8, 1)
          : formatSignedPercentage(0, 1),
        color: 'text-warning',
      ...
      },
      {
        ...
        trend: 'up' as const,
        trendValue: formatSignedPercentage(1.5, 1),
        color: 'text-success',
        bgColor: 'bg-success/10'
      },
      {
        label: 'كفاءة التكلفة',
        value: formatPercentageValue(financialData.kpis.costEfficiency, 1),
        trend: 'up' as const,
        trendValue: formatSignedPercentage(1.5, 1),
        color: 'text-success',
        bgColor: 'bg-success/10'
      },
      {
        ...
        trend: 'down' as const,
        trendValue: formatSignedPercentage(-5.2, 1),
        color: 'text-warning',
        bgColor: 'bg-warning/10'
      },
      {
        label: 'تقارير قيد التوليد',
        value: formatIntegerValue(metrics.reports.generatingCount),
        trend: metrics.reports.generatingCount > 0 ? ('up' as const) : ('down' as const),
        trendValue: formatSignedInteger(metrics.reports.generatingCount > 0 ? 1 : 0),
        color: 'text-info',
        bgColor: 'bg-info/10'
      },
      {
        label: 'مشاريع تحت التنفيذ',
        value: formatIntegerValue(metrics.projects.activeCount),
        ...
        trendValue: metrics.projects.activeCount === 0
          ? 'لا مشاريع نشطة'
          : `${formatIntegerValue(metrics.projects.onTrackCount)}/${formatIntegerValue(metrics.projects.activeCount)} على المسار`,
        color: 'text-primary',
        bgColor: 'bg-primary/10'
      },
      {
        label: 'منافسات نشطة',
        value: formatIntegerValue(metrics.tenders.activeCount),
        trend: metrics.tenders.upcomingDeadlines > 0 ? ('down' as const) : ('up' as const),
        trendValue: metrics.tenders.upcomingDeadlines > 0
          ? `${formatIntegerValue(metrics.tenders.upcomingDeadlines)} خلال ${formatIntegerValue(14)} يوماً`
          : 'لا مواعيد قريبة',
        color: 'text-accent',
        bgColor: 'bg-accent/10'
      }
    ]
  }, [financialData, suppliersData, metrics, formatCurrencyValue, formatPercentageValue, formatSignedPercentage, formatIntegerValue, formatSignedInteger])

  // الإجراءات السريعة
  const quickActions = useMemo(() => {
    const formatCount = (count: number, loadingState: boolean) => (loadingState ? '…' : formatIntegerValue(count))
  ...
  }, [handleRefresh, activateTab, metrics, invoicesState.isLoading, budgetsState.isLoading, reportsState.isLoading, projectsState.isLoading, tendersState.isLoading, formatCurrencyValue, formatIntegerValue])
    ...
    () => [
      {
        ...
        subValue: `من ${formatIntegerValue(financialData.revenue.projects)} مشاريع`,
        icon: TrendingUp,
        color: 'text-success',
        bgColor: 'bg-success/10',
        trend: financialData.revenue.growth >= 0 ? ('up' as const) : ('down' as const),
        trendValue: formatSignedPercentage(financialData.revenue.growth, 1),
      },
      {
        label: 'المصروفات الشهرية',
        value: formatCurrencyValue(financialData.expenses.monthly),
        subValue: 'تشغيلية ومشاريع',
        icon: TrendingDown,
        color: 'text-destructive',
        bgColor: 'bg-destructive/10',
        trend: 'down' as const,
        trendValue: formatSignedPercentage(-2.8, 1),
      },
      {
        label: 'التدفق النقدي',
        value: formatCurrencyValue(financialData.cashFlow.incoming - financialData.cashFlow.outgoing),
        subValue: 'صافي التدفق',
        icon: Wallet,
        color: 'text-primary',
        bgColor: 'bg-primary/10',
        trend: 'up' as const,
        trendValue: formatSignedPercentage(12.5, 1),
      },
      {
        label: 'دورة الدفع',
        value: `${formatNumberValue(financialData.kpis.paymentCycle, {
          locale: DEFAULT_LOCALE,
          minimumFractionDigits: 0,
          maximumFractionDigits: 0,
        })} يوم`,
        subValue: 'متوسط التحصيل',
        icon: Clock,
        color: 'text-secondary-foreground',
        bgColor: 'bg-secondary/10',
        trend: 'down' as const,
        trendValue: `${formatSignedInteger(-3)} أيام`,
      },
    ],
    [DEFAULT_LOCALE, financialData, formatCurrencyValue, formatIntegerValue, formatNumberValue, formatSignedInteger, formatSignedPercentage]
  )
  ...
  const resolveTransactionStatusBadge = (status: TransactionStatus): { status: FinancialBadgeStatus; label: string } =>
    status === 'completed'
      ? { status: 'success', label: 'مكتملة' }
      : { status: 'warning', label: 'معلقة' }

  const resolveSupplierStatusBadge = (status: string): { status: FinancialBadgeStatus; label: string } =>
    status === 'active'
      ? { status: 'success', label: 'نشط' }
      : { status: 'default', label: 'غير نشط' }

  const resolveInvoiceStatusBadge = (status: string): { status: FinancialBadgeStatus; label: string } => {
    switch (status) {
      case 'draft':
  ...
  const resolveBudgetUtilizationBadge = (utilization: number): { status: FinancialBadgeStatus; label: string } => {
  ...
  const resolveProjectRiskBadge = (health: string): { status: FinancialBadgeStatus; label: string } =>
    health === 'red'
      ? { status: 'overdue', label: 'حرجة' }
      : { status: 'warning', label: 'عالية المخاطر' }

  const resolveTenderDeadlineBadge = (daysLeft: number | null | undefined): { status: FinancialBadgeStatus; label: string } => {
  ...
  const resolveProjectStatusBadge = (status: string): { status: FinancialBadgeStatus; label: string } => {
    switch (status) {
      case 'active':
  ...
  const resolveTenderStatusBadge = (status: string): { status: FinancialBadgeStatus; label: string } => {
    switch (status) {
      case 'new':
  ...
  const resolveReportStatusBadge = (status: string): { status: FinancialBadgeStatus; label: string } => {
    switch (status) {
      case 'completed':
        return { status: 'success', label: 'مكتمل' }
      case 'generating':
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                ...
                <StatusBadge status={statusInfo.status} label={statusInfo.label} size="sm" showIcon={false} className="shadow-none" />
              </div>
            </div>
  ...
  headerExtra={<DetailStats stats={analysisStats} />}
    >
      {showRefreshingState && (
  ...
  <Tabs value={activeTab} onValueChange={handleTabValueChange} className="w-full" dir="rtl">
        ...
        {financialAlert && (
          <div className="mb-6">
            <InlineAlert variant={financialAlert.variant} title={financialAlert.title} description={financialAlert.description} />
          </div>
        )}
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-3 border-t border-border">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-success">{formatSignedPercentage(financialData.revenue.growth, 1)}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="border-border shadow-sm">
            ...
            <CardContent>
                  ...
                  <tbody>
                    {suppliersData.map((supplier, index) => (
                      ...
                      >
                        ...
                        <td className="py-4">
                          <StatusBadge status="info" label={supplier.category} size="sm" showIcon={false} className="shadow-none" />
                        </td>
                        ...
                        <td className="py-4">
                          {(() => {
                            const { status, label } = resolveSupplierStatusBadge(supplier.status)
                            return <StatusBadge status={status} label={label} size="sm" showIcon={false} className="shadow-none" />
                          })()}
                        </td>
                        <td className="py-4">
                          <div className="flex gap-2">
              ...
              <div className="flex justify-between items-center mt-4 pt-4 border-t border-border">
                <div className="text-sm text-muted-foreground">
                  عرض {formatIntegerValue(suppliersData.length)} من {formatIntegerValue(suppliersData.length)} مورد
                </div>
                  ...
                  budgetsAtRisk.map(budget => (
                    <div key={budget.id} className="flex flex-col gap-2 p-3 rounded-lg border border-border">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {(() => {
                          const { status, label } = resolveBudgetUtilizationBadge(budget.utilizationPercentage)
                          return <StatusBadge status={status} label={label} size="sm" showIcon={false} className="shadow-none" />
                        })()}
                      </div>
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-medium">{formatPercentageValue(financialData.profitability.gross, 1)}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-medium">{formatPercentageValue(financialData.profitability.net, 1)}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-medium">{formatPercentageValue(financialData.kpis.costEfficiency, 1)}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-medium">{formatPercentageValue(financialData.profitability.roi, 1)}</span>
                </div>
              ...
              <CardContent className="space-y-4">
                        ...
                        {displayedProjects.map((project) => (
                          <tr key={project.id} className="border-b border-border hover:bg-muted/40">
                            <td className="py-3">
                              ...
                              <div className="text-xs text-muted-foreground">
                                العميل: {project.client ?? 'غير محدد'} • المدير: {project.manager ?? '—'}
                              </div>
                            </td>
                            <td className="py-3">
                              {(() => {
                            ...
                            <td className="py-3">
                              <div className="flex items-center gap-3">
                                <div className="w-24">
                                  <Progress value={Math.min(100, Math.max(0, Number(project.progress ?? 0)))} className="h-2" />
                                </div>
                            ...
                            <td className="py-3 font-medium text-foreground text-right">
                              {formatCurrencyValue(Number(project.contractValue ?? project.value ?? 0))}
                            </td>
                ...
                {projectItems.length > 0 && !projectsState.isLoading && (
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-4 border-t border-border">
                    <div className="text-sm text-muted-foreground">
                      عرض {formatIntegerValue(displayedProjects.length)} من {formatIntegerValue(projectItems.length)} مشروعاً
                    </div>
                    <Button variant="outline" size="sm" onClick={() => onSectionChange('projects')} className="gap-2">
            ...
            <Card className="border-border shadow-sm">
              ...
              <CardContent className="space-y-4">
                {tendersState.isLoading ? (
                  <div className="text-sm text-muted-foreground">جارٍ تحميل بيانات المنافسات...</div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {displayedTenders.map((tender) => {
                          const deadlineLabel = formatDateValue(tender.deadline ?? tender.submissionDate, { locale: 'ar-EG' })
                          const winChanceValue = Number(tender.winChance ?? tender.completionPercentage ?? 0)
                          return (
                            <tr key={tender.id} className="border-b border-border hover:bg-muted/40">
                              <td className="py-3">
                                <div className="font-medium text-foreground">{tender.name}</div>
                                <div className="text-xs text-muted-foreground">العميل: {tender.client ?? 'غير محدد'}</div>
                              </td>
                              <td className="py-3">
                                {(() => {
                              ...
                              <td className="py-3">
                                <div className="flex flex-col items-start gap-1">
                                  <span className="text-sm font-medium text-foreground">{deadlineLabel}</span>
                                  {(() => {
                                    const { status, label } = resolveTenderDeadlineBadge(tender.daysLeft)
                                    return <StatusBadge status={status} label={label} size="sm" showIcon={false} className="shadow-none" />
                                  })()}
                                </div>
                              </td>
                              <td className="py-3 font-medium text-foreground text-right">
                                {formatCurrencyValue(Number(tender.value ?? tender.totalValue ?? 0))}
                              </td>
                              <td className="py-3">
                                <div className="flex items-center gap-3 justify-end">
                                  <div className="w-24">
                                    <Progress value={Math.min(100, Math.max(0, winChanceValue))} className="h-2" />
                                  </div>
                                  <span className="text-sm font-medium text-foreground">
                ...
                {tenderItems.length > 0 && !tendersState.isLoading && (
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-4 border-t border-border">
                    <div className="text-sm text-muted-foreground">
                      عرض {formatIntegerValue(displayedTenders.length)} من {formatIntegerValue(tenderItems.length)} منافسة
                    </div>
                    <Button variant="outline" size="sm" onClick={() => onSectionChange('tenders')} className="gap-2">
              ...
              <CardContent className="space-y-3 text-sm text-muted-foreground">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-foreground">
                    {formatPercentageValue(metrics.tenders.performance.winRate ?? metrics.tenders.averageWinChance, 0)}
                  </span>
                </div>
                <div className="flex justify-between">
