    ...
    >
      <div className="flex items-start justify-between gap-3">
        <span className="flex items-start gap-2">
          <span className={cn('mt-0.5 flex h-6 w-6 items-center justify-center rounded-full bg-card/70', style.accent)}>
            {icon ?? style.icon}
          </span>
        ...
        {actions ? <span className="flex flex-shrink-0 items-center gap-2 text-xs">{actions}</span> : null}
      </div>
      {children ? <div className="text-xs leading-snug text-muted-foreground">{children}</div> : null}
    </div>
  );
};
