...
export function Sidebar() {
  ...
  const resolvedActiveSection = FINANCIAL_CHILD_SECTIONS.has(activeSection) ? 'financial' : activeSection
        ...
        <div className="p-4 border-b border-border">
          <div className="bg-card border border-border/40 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                {formatInteger(totalNotifications)}
              </Badge>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="text-center">
                <div className="font-bold text-success">{formatInteger(activeProjects)}</div>
                <div className="text-muted-foreground">مشاريع نشطة</div>
              </div>
              <div className="text-center">
                <div className="font-bold text-warning">{formatInteger(activeTenders)}</div>
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-border">
          <div className="bg-card border border-border/40 rounded-lg p-3">
            ...
            <div className="text-xs text-muted-foreground">{systemHealthStatusMessage}</div>
          ...
          <div className="mt-3 text-center">
            <p className="text-xs text-muted-foreground">
              {formatInteger(totalProjects)} مشاريع • {formatInteger(totalClients)} عملاء • {formatInteger(totalTenders)} منافسات
            </p>
          </div>
        </div>
      )}
