...
export const CostBaselineTotals: React.FC<CostBaselineTotalsProps> = ({
  ...
  const varianceColor = varianceValue > 0 ? 'text-destructive' : varianceValue < 0 ? 'text-success' : 'text-muted-foreground';
  ...
  return (
    <div className={"grid grid-cols-3 border border-border rounded-md overflow-hidden text-xs bg-card shadow-sm " + (className ?? '')}>
