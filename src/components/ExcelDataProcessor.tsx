...
export class ExcelDataProcessor {
  ...
  static processProjectsData(rawData: ExcelRow[]): ImportedProject[] {
    return rawData.map((row, index) => {
      const budget = this.normalizeNumber(this.pickFirstValue(row, ['budget', 'الميزانية', 'Budget']), 0)
      ...
      return {
        ...
        priority: this.normalizePriority(this.pickFirstValue(row, ['priority', 'الأولوية', 'Priority'])),
        ...
        budget,
        spent,
        startDate: this.normalizeDate(this.pickFirstValue(row, ['startDate', 'تاريخ البداية', 'Start Date'])),
        endDate: this.normalizeDate(this.pickFirstValue(row, ['endDate', 'تاريخ النهاية', 'End Date'])),
        manager: this.ensureString(this.pickFirstValue(row, ['manager', 'المدير', 'Manager']), 'غير محدد'),
  ...
  static processClientsData(rawData: ExcelRow[]): ImportedClient[] {
    return rawData.map((row, index) => {
      ...
      return {
        ...
        projects: this.normalizeNumber(this.pickFirstValue(row, ['projects', 'المشاريع', 'Projects']), 0, 1000),
        totalValue: this.normalizeNumber(
          this.pickFirstValue(row, ['totalValue', 'القيمة الإجمالية', 'Total Value']),
          0
        ),
        status: this.ensureString(this.pickFirstValue(row, ['status', 'الحالة', 'Status']), 'active'),
        ...
        location: this.ensureString(this.pickFirstValue(row, ['location', 'الموقع', 'Location']), 'غير محدد'),
        contact: this.ensureString(this.pickFirstValue(row, ['contact', 'جهة الاتصال', 'Contact']), 'غير محدد'),
        phone: this.ensureString(this.pickFirstValue(row, ['phone', 'الهاتف', 'Phone']), ''),
        email: this.ensureString(this.pickFirstValue(row, ['email', 'البريد الإلكتروني', 'Email']), ''),
        ...
        completedProjects: this.normalizeNumber(
          this.pickFirstValue(row, ['completedProjects', 'المشاريع المكتملة', 'Completed Projects']),
          0,
          100
        )
      }
  ...
  static processTendersData(rawData: ExcelRow[]): ImportedTender[] {
    ...
    return rawData.map((row, index) => {
      ...
      return {
        ...
        value: this.normalizeNumber(this.pickFirstValue(row, ['value', 'القيمة', 'Value']), 1_000_000),
        ...
        priority: this.normalizePriority(this.pickFirstValue(row, ['priority', 'الأولوية', 'Priority'])),
        team: this.ensureString(this.pickFirstValue(row, ['team', 'الفريق', 'Team']), 'فريق المنافسات'),
        manager: this.ensureString(this.pickFirstValue(row, ['manager', 'المدير', 'Manager']), 'غير محدد'),
        ...
        lastUpdate: this.currentDateString(),
        category: this.ensureString(this.pickFirstValue(row, ['category', 'التصنيف', 'Category']), 'عام'),
        location: this.ensureString(this.pickFirstValue(row, ['location', 'الموقع', 'Location']), 'غير محدد'),
  ...
  static processInventoryData(rawData: ExcelRow[]): ImportedInventoryItem[] {
    return rawData.map((row, index) => {
      ...
      const status: InventoryStatus = currentStock <= minStock
        ? 'warning'
      ...
      return {
        ...
        category: this.ensureString(this.pickFirstValue(row, ['category', 'التصنيف', 'Category']), 'مواد عامة'),
        currentStock,
        minStock,
        maxStock,
        unit: this.ensureString(this.pickFirstValue(row, ['unit', 'الوحدة', 'Unit']), 'قطعة'),
        location: this.ensureString(this.pickFirstValue(row, ['location', 'الموقع', 'Location']), 'المخزن الرئيسي'),
        supplier: this.ensureString(this.pickFirstValue(row, ['supplier', 'المورد', 'Supplier']), 'مورد غير محدد'),
        price,
        totalValue,
