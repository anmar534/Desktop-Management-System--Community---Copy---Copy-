...
import type { Alert, Activity } from '@/components/dashboard/enhanced/EnhancedDashboardLayout';
 ...
 */
export const useDashboardAlerts = (options: UseDashboardAlertsOptions = {}): UseDashboardAlertsReturn => {
  const {
    autoRefresh = true,
   ...
   * تحميل جميع البيانات
   */
  const loadAllData = useCallback(async (showLoading = true): Promise<void> => {
   ...
   * إضافة تنبيه جديد
   */
  const addAlert = useCallback(async (alert: Omit<Alert, 'id' | 'timestamp'>): Promise<string> => {
    try {
   ...
   * تسجيل نشاط جديد
   */
  const logActivity = useCallback(async (activity: Omit<Activity, 'id' | 'timestamp'>): Promise<string> => {
    try {
   ...
   */
  const getActivitiesByType = useCallback((type: Activity['type']): Activity[] => {
   ...
   */
  const getActivitiesByUser = useCallback((user: string): Activity[] => {
