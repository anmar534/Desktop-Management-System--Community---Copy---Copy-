...
const ACTIVE_TENDER_STATUSES = new Set<Tender['status']>([
  'new',
  'under_action',
  'ready_to_submit',
  'submitted',
...
const buildProjectMetrics = (
  projects: Project[],
  analyzer: ProjectCostAnalyzerLike = ProjectCostAnalyzer,
): ProjectMetrics => {
  let activeCount = 0
  let completedCount = 0
  let delayedCount = 0
...
const buildTenderMetrics = (tenders: Tender[]): TenderMetrics => {
  ...
  const getDaysUntilDeadline = (tender: Tender): number => {
    const normalizedDaysLeft =
      typeof tender.daysLeft === 'number' ? tender.daysLeft : Number(tender.daysLeft)
  ...
  for (const tender of tenders) {
    ...
    if (
      daysUntilDeadline <= 7 &&
      daysUntilDeadline >= 0 &&
      ACTIVE_TENDER_STATUSES.has(tender.status)
    ) {
      upcomingDeadlines += 1
    }
  }
  ...
  // استخدام selectors لجميع الحسابات
  ...
  return {
    ...
    activeCount: selectActiveTendersCount(tenders),
    submittedCount: selectSubmittedTendersCount(tenders),
    wonCount: selectWonTendersCount(tenders),
    lostCount: selectLostTendersCount(tenders),
    upcomingDeadlines,
    averageWinChance,
    performance,
  }
...
export const selectFinancialHighlights = (params: {
  ...
  const getInvoiceTimestamp = (invoice: Invoice) =>
    new Date(invoice.dueDate ?? invoice.issueDate ?? 0).getTime()
  ...
  const getReportTimestamp = (report: FinancialReport) =>
    new Date(report.createdAt ?? report.completedAt ?? 0).getTime()
  ...
  const projectsAtRisk = params.projects
    .filter(
      (project) =>
        project.health === 'red' || project.riskLevel === 'high' || project.priority === 'critical',
    )
...
export const selectDashboardMetrics = (params: {
  ...
  // استخدام selectors بدلاً من TenderMetricsService
  const tenderPerformance: TenderMetricsSummary = {
    total: params.tenders.length,
    submitted: selectSubmittedTendersCount(params.tenders),
    won: selectWonTendersCount(params.tenders),
    lost: selectLostTendersCount(params.tenders),
    waiting: selectWaitingTendersCount(params.tenders),
    underReview: selectUnderReviewTendersCount(params.tenders),
    submittedValue: selectSubmittedTendersValue(params.tenders),
    wonValue: selectWonTendersValue(params.tenders),
    lostValue: selectLostTendersValue(params.tenders),
    winRate: selectWinRate(params.tenders),
    averageCycleDays: selectAverageCycleDays(params.tenders),
  }
  const tenderMonthly = selectTenderMonthlyStats(params.tenders)

  const startingBalance = params.bankAccounts.reduce(
    (total, account) =>
      total + normalizeCurrency(account.currentBalance, account.currency, currencyOptions),
    0,
  )
  ...
  const activeProjects = params.projects.filter((project) =>
    ACTIVE_PROJECT_STATUSES.has(project.status),
  ).length
  const openTenders = params.tenders.filter((tender) =>
    ACTIVE_TENDER_STATUSES.has(tender.status),
  ).length
