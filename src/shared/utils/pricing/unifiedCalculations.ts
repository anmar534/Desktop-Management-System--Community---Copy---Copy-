...
} from '@/application/services/developmentStatsService'
...
type PurchaseOrderStatsSnapshot = Awaited<
  ReturnType<typeof purchaseOrderService.getPurchaseOrderStats>
>
...
const calculatePricingTotals = (
  snapshot?: TenderPricingSnapshot,
...
export const TenderCalculations = {
  calculateMonthlyStats: (tenders: Tender[], year?: number, month?: number): MonthlyTenderStats => {
    ...
    for (const tender of tenders) {
      ...
      if (
        !relevantDate ||
        relevantDate.getFullYear() !== targetYear ||
        relevantDate.getMonth() !== targetMonth
      ) {
        continue
      }
    ...
    const winRate = WinRateCalculations.calculateWinRateForPeriod(
      monthlyTenders,
      targetYear,
      targetMonth + 1,
    )
  ...
  mergeDevelopmentStats: (
    tenders: Tender[],
    year?: number,
    month?: number,
  ): DevelopmentMergedStats => {
    try {
      const developmentStats: DevelopmentStatsSnapshot =
        developmentStatsService.getDevelopmentStats()
      const calculatedStats = TenderCalculations.calculateMonthlyStats(tenders, year, month)

      const mergedSubmitted = Math.max(
        calculatedStats.submitted,
        developmentStats.submittedTenders ?? 0,
      )
      ...
      const mergedWonValue = Math.max(
        calculatedStats.wonValue,
        developmentStats.wonTendersValue ?? 0,
      )
      ...
      const additionalLostTenders = Math.max(
        0,
        (developmentStats.lostTenders ?? 0) - calculatedStats.won,
      )
      ...
      const relevantTenders = allTenders.filter(
        (tender) => tender.status && isSubmittedStatus(tender.status),
      )
      ...
      return {
        ...
        winRate,
        documentROI:
          totalBookletsCost > 0 ? Math.round((mergedWonValue / totalBookletsCost) * 100) / 100 : 0,
        syntheticData,
      }
    } catch (error) {
  ...
  mergeWithPurchaseOrders: async (
    stats: DevelopmentMergedStats,
  ): Promise<PurchaseOrderMergedStats> => {
    try {
      const purchaseStats: PurchaseOrderStatsSnapshot =
        await purchaseOrderService.getPurchaseOrderStats()
...
export const UnifiedCalculations = {
  ...
  validateWinRateCalculation: (tenders: Tender[]) =>
    WinRateCalculations.validateWinRateCalculation(tenders),
...
export const ProjectCalculations = {
  createProjectFromTender: (tender: Tender, pricingData?: TenderPricingSnapshot | null) => {
    ...
    const { cost: estimatedCost, profit: estimatedProfit } = calculatePricingTotals(
