...
import {
  ...
  DEFAULT_PERCENTAGES,
} from '@/application/services/pricingEngine'
...
export type RawPricingInput =
  | [string, PricingItemInput][]
  | Record<string, PricingItemInput>
  | undefined
...
const toResourceArray = (value: unknown): PricingResource[] =>
  Array.isArray(value) ? (value as PricingResource[]) : []
...
export const PRICING_FLAGS = {
  ...
  USE_ENGINE_AUTHORING: readBoolEnv(
    ['VITE_FEATURE_ENGINE_AUTHORING', 'FEATURE_ENGINE_AUTHORING'],
    true,
  ), // مسار التحرير (Authoring Mode)
  // Default flipped to true as part of domain adoption rollout (2025-09) – يمكن تعطيلها عبر متغيرات البيئة
  USE_DOMAIN_PRICING_UI: readBoolEnv(
    ['VITE_FEATURE_DOMAIN_PRICING_UI', 'FEATURE_DOMAIN_PRICING_UI'],
    true,
  ), // (Phase 2.5) تفعيل واجهة التسعير عبر محرك الدومين
...
export function getCanonicalDescription(
...
export function dedupePricingItems(items: EnrichedPricingItem[]): EnrichedPricingItem[] {
  ...
  const pickPrimary = (entries: EnrichedPricingItem[]): EnrichedPricingItem => {
    ...
    for (const candidate of entries) {
      ...
      const arraysLength =
        (Array.isArray(candidate.materials) ? candidate.materials.length : 0) +
      ...
      const descriptionScore =
        desc && normalizedDesc !== 'البند' && !normalizedDesc.includes('غير محدد')
  ...
  const computeBreakdownFromArrays = (
    primary: EnrichedPricingItem,
    merged: PricingBreakdownArrays,
  ): ItemBreakdown => {
  ...
  for (const id of order) {
    ...
    const mergedArrays: PricingBreakdownArrays = {
      ...
      subcontractors: mergeArrays(
        groupedEntries.map((entry) => toResourceArray(entry.subcontractors)),
      ),
    }

    const hasDetailedArrays =
      mergedArrays.materials.length +
        mergedArrays.labor.length +
        mergedArrays.equipment.length +
        mergedArrays.subcontractors.length >
      0
    const breakdown = hasDetailedArrays
      ? computeBreakdownFromArrays(primary, mergedArrays)
      : primary.breakdown

    const resolvedItemNumber =
      primary.itemNumber ?? groupedEntries.find((entry) => entry.itemNumber)?.itemNumber
    const resolvedUnitCandidate =
      typeof primary.unit === 'string'
        ? primary.unit
        : groupedEntries.find((entry) => typeof entry.unit === 'string')?.unit
    const resolvedUnit =
      resolvedUnitCandidate && resolvedUnitCandidate !== '-' && resolvedUnitCandidate !== '—'
        ? resolvedUnitCandidate
        : primary.unit
    const resolvedDescription =
      typeof primary.description === 'string' && primary.description.trim().length > 0
        ? primary.description
        : groupedEntries.find(
