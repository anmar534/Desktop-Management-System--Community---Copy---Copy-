...
type StoreMethod<TArgs extends unknown[], TResult> =
...
const hasElectronStore = (
  api: ElectronAPI | undefined,
): api is ElectronAPI & { store: ElectronStore } =>
...
class ElectronStoreInterface {
  ...
  private async loadCacheFromStore(
    options: { reset?: boolean; dispatchEvent?: boolean } = {},
  ): Promise<void> {
              ...
              if (legacyValue !== null && legacyValue !== undefined) {
                ...
                if (legacyDecoded.value !== undefined) {
                  ...
                  try {
                    ...
                    console.warn(
                      `⚠️ Failed to remove legacy electron-store value for ${key}:`,
                      error,
                    )
                  }
  ...
  private async persistCachedEntry(
    key: string,
    cachedValue: PersistedValue,
  ): Promise<StorageFlushEntry> {
    const target: StorageFlushEntry['target'] = isSensitiveKey(key)
      ? 'secure'
      : isElectron()
        ? 'electron'
        : 'browser'
...
export function installLegacyStorageGuard(): void {
  ...
  const logOnce = (key: string, action: 'read' | 'write' | 'remove'): void => {
    ...
    warnedKeys.add(key)
    console.debug(
      `⚠️ Legacy storage access attempted: ${key} (${action}). Access is deprecated and blocked.`,
    )
  }
