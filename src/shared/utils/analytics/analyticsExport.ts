...
export function exportBidPerformances(
  ...
  const data = performances.map((performance) => [
    ...
    performance.result === 'won' ? 'فائز' : performance.result === 'lost' ? 'خاسر' : 'معلق',
...
export function exportCompetitors(
  ...
  const data = competitors.map((competitor) => [
    competitor.name || '-',
    competitor.type === 'private' ? 'خاص' : competitor.type === 'government' ? 'حكومي' : 'مختلط',
    competitor.region || '-',
    ...
    competitor.threatLevel === 'high'
      ...
      : competitor.threatLevel === 'medium'
        ? 'متوسط'
        : 'منخفض',
    competitor.marketPosition === 'leader'
      ? 'رائد'
      : competitor.marketPosition === 'challenger'
        ? 'منافس'
        : competitor.marketPosition === 'follower'
          ? 'تابع'
          : 'غير معروف',
 ...
 */
export function exportMarketTrends(trends: MarketTrend[], options: ExportOptions): ExportData {
  const headers = [
    'عنوان الاتجاه',
    'الوصف',
    'الفئة',
  ...
  const data = trends.map((trend) => [
    ...
    trend.description || '-',
    trend.category || '-',
    trend.trendType === 'growth' ? 'نمو' : trend.trendType === 'decline' ? 'انخفاض' : 'استقرار',
    trend.impactLevel === 'high' ? 'عالي' : trend.impactLevel === 'medium' ? 'متوسط' : 'منخفض',
