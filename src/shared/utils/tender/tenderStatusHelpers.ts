...
import { Trophy, XCircle, Clock, CheckCircle, AlertTriangle, FileText, Zap } from 'lucide-react'
...
export const getTenderStatusInfo = (
  status: TenderStatus | string | undefined | null,
): TenderStatusInfo => {
...
type TenderProgressSource = Pick<Tender, 'status'> &
  Partial<
    Pick<
      Tender,
      'pricedItems' | 'itemsPriced' | 'totalItems' | 'technicalFilesUploaded' | 'daysLeft'
    >
  > & {
...
const URGENT_STATUSES: readonly TenderStatus[] = ['new', 'under_action', 'ready_to_submit'] as const
