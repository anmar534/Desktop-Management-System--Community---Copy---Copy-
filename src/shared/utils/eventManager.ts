...
// كلاس إدارة الأحداث
class EventManager {
  private listeners: Map<SystemEventType, Set<EventListener<unknown>>> = new Map<
    SystemEventType,
  ...
  private logAuditedEvent(eventData: SystemEventData<unknown>): void {
    ...
    recordAuditEvent({
      ...
      level: config.level,
      actor:
        typeof eventData.source === 'string' && eventData.source.trim() !== ''
          ? eventData.source
          : 'system',
