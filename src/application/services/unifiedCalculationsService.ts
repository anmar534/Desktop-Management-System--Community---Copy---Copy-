...
export class UnifiedCalculationsService {
  ...
  private isTestEnvironment(): boolean {
    const hasVitestGlobal = typeof (globalThis as unknown as { vitest?: unknown })?.vitest !== 'undefined'
    if (hasVitestGlobal) {
      return true
    }
  ...
  private ensureCacheWarm() {
    ...
    this.loadingPromise = this.refreshCaches().catch(error => {
   ...
   */
  public calculateWinRateTrend(periodMonths = 6): { current: number; previous: number; trend: 'up' | 'down' | 'stable' } {
  ...
  public analyzeProjects(projects?: Project[]): ProjectAnalytics {
    ...
    const totalContractValue = dataset.reduce((sum: number, p: Project) => sum + (p.contractValue || 0), 0);
    const totalActualCost = dataset.reduce((sum: number, p: Project) => sum + (p.actualCost || 0), 0);
  ...
  public calculateProjectEfficiency(project: Project): {
    ...
    const costEfficiency = project.actualCost && project.actualCost > 0
  ...
  const totalInvestment = projects.reduce((sum: number, p: Project) => sum + (p.estimatedCost || 0), 0);
   ...
   */
  public calculateCashFlow(months = 12): { month: string; inflow: number; outflow: number; net: number }[] {
  ...
  public calculateKPIs(): KPIMetrics {
    ...
    // معدل تجاوز التكلفة
    const projects = this.resolveProjects();
    const projectsWithCostOverrun = projects.filter((p: Project) => 
      p.actualCost && p.estimatedCost && p.actualCost > p.estimatedCost
    );
    const costOverrunRate = projects.length > 0 
      ? (projectsWithCostOverrun.length / projects.length) * 100 
      : 0;
    ...
    const timeOverrunRate = (completedProjects.length + delayedProjects.length) > 0 
    ...
    const clientSatisfactionScore = completedProjects.length > 0 
    ...
    return {
      ...
      averageProjectValue: projectAnalytics.totalContractValue / Math.max(projectAnalytics.totalProjects, 1),
   ...
   */
  public getIndicatorColor(value: number, thresholds: { good: number; warning: number }): 'green' | 'yellow' | 'red' {
