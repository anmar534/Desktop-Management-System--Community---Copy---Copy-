...
export interface ProjectBOQEnvelopeMeta {
  ...
  itemStats?: { total: number; modified: number; unmodified: number; added: number; removed: number };
...
const ensureProjectCostItem = (item: ProjectCostItem | null, errorMessage: string): ProjectCostItem => {
  if (!item) {
...
function computeItemVariance(item: ProjectCostItem) {
  ...
  const pct = item.estimated.totalPrice > 0 ? (value / item.estimated.totalPrice) * 100 : (item.actual.totalPrice > 0 ? 100 : 0);
...
function recomputeTotals(boq: ProjectBOQData) {
  ...
  const variancePct = estimatedTotal > 0 ? (varianceTotal / estimatedTotal) * 100 : (actualTotal > 0 ? 100 : 0);
...
export const projectCostService = {
  ...
  computeActualCostDecomposition(projectId: string) {
    ...
    if (!env?.draft) return { base: 0, administrative: 0, operational: 0, profit: 0, actualTotal: 0 };
    let base = 0; let administrative = 0; let operational = 0; let profit = 0; let actualTotal = 0;
    for (const it of env.draft.items) {
      ...
      if (Array.isArray(side.breakdownTables) && side.breakdownTables.length > 0) {
        for (const tbl of side.breakdownTables) {
          if (!tbl?.breakdown) continue;
          itemBase += sumRows(tbl.breakdown.materials) + sumRows(tbl.breakdown.labor) + sumRows(tbl.breakdown.equipment) + sumRows(tbl.breakdown.subcontractors);
        }
      } else if (side?.breakdown) {
        itemBase += sumRows(side.breakdown.materials) + sumRows(side.breakdown.labor) + sumRows(side.breakdown.equipment) + sumRows(side.breakdown.subcontractors);
      }
      const adminPct = side?.additionalPercentages?.administrative ?? 0;
      const opPct = side?.additionalPercentages?.operational ?? 0;
      const profitPct = side?.additionalPercentages?.profit ?? 0;
  ...
  addActualBreakdownTable(projectId: string, itemId: string, name?: string): ActualBreakdownTable | null {
  ...
  renameActualBreakdownTable(projectId: string, itemId: string, tableId: string, newName: string) {
    this.saveDraft(projectId, draft => {
      const tbl = draft.items.find(i=>i.id===itemId)?.actual.breakdownTables?.find(t=>t.id===tableId);
      if (tbl) { tbl.name = newName; tbl.updatedAt = new Date().toISOString(); }
    });
  },
  ...
  upsertActualBreakdownRow(projectId: string, itemId: string, tableId: string, section: keyof CostBreakdownSet, row: Partial<BreakdownRow> & { id?: string }) {
    this.saveDraft(projectId, draft => {
      const it = draft.items.find(i=>i.id===itemId);
      if (!it) return;
      ...
      const rid = toNonEmptyString(row.id) ?? `row_${Date.now()}_${Math.random().toString(36).slice(2,5)}`;
      let target = list.find(r=>r.id===rid);
      if (!target) {
        const defaultName = toNonEmptyString(row.name) ?? 'عنصر';
        target = { id: rid, name: defaultName, quantity: 0, unitCost: 0, totalCost: 0, origin: 'actual-only' };
        list.push(target);
      }
  ...
  removeActualBreakdownRow(projectId: string, itemId: string, section: keyof CostBreakdownSet, rowId: string) {
    this.saveDraft(projectId, draft => {
      const it = draft.items.find(i=>i.id===itemId);
  ...
  initEnvelope(projectId: string, opts: { sourceTenderId?: string; strategy?: 'initial' } = {}): ProjectCostEnvelope {
    const all = loadAll();
  ...
  saveDraft(projectId: string, mutate: (draft: ProjectBOQData) => void | ProjectBOQData): ProjectBOQData {
    const all = loadAll();
    ...
    void (async () => {
      try {
        ...
        if (project) {
          ...
          const persisted = (await projectRepository.update(projectId, updated)) ?? { ...project, ...updated };
  ...
  async syncEstimatedFromTender(projectId: string, tenderId?: string) {
    ...
    for (const item of env.draft.items) {
      ...
      if (item.estimated.quantity !== newQty || item.estimated.unitPrice !== newUnit || item.estimated.totalPrice !== newTotal) {
        item.estimated.quantity = newQty;
        item.estimated.unitPrice = newUnit;
        item.estimated.totalPrice = newTotal;
        changed = true;
      }
      if (item.estimated.quantity > 1 && Math.abs((item.estimated.unitPrice ?? 0) * item.estimated.quantity - (item.estimated.totalPrice ?? 0)) > 0.0001) {
        // keep user-provided adjustments
      } else if (item.estimated.quantity > 1 && item.estimated.unitPrice === item.estimated.totalPrice) {
      ...
      const newCount = newBreak.materials.length + newBreak.labor.length + newBreak.equipment.length + newBreak.subcontractors.length;
      if (newCount > 0) {
        const sections: (keyof CostBreakdownSet)[] = ['materials', 'labor', 'equipment', 'subcontractors'];
        const oldSum = sections.reduce((sum, section) => sum + sumBreakdownRows(item.estimated.breakdown[section]), 0);
  ...
  async mergeFromTender(projectId: string, tenderId: string) {
    ...
    for (const src of tenderBOQ.items) {
      ...
      if (!target) {
        ...
        const actualQuantity = pickNumber(src.actual?.quantity, src.estimated?.quantity, src.quantity) ?? 0;
        const actualUnitPrice = pickNumber(src.actual?.unitPrice, src.estimated?.unitPrice, src.unitPrice) ?? 0;
        const actualTotalPrice = pickNumber(src.actual?.totalPrice, src.estimated?.totalPrice, src.totalPrice) ?? 0;
        ...
        const newItem: ProjectCostItem = {
          ...
          actual: {
            ...
            additionalPercentages: src.actual?.additionalPercentages ?? src.estimated?.additionalPercentages ?? {}
          },
        ...
        if (!target.state.isModified) {
          ...
          if (totalBreakdownRows > 0) {
            ...
            const actualHasRows =
              ...
              target.actual.breakdown.subcontractors.length > 0;
            if (!actualHasRows) {
    ...
    bus.emit(APP_EVENTS.COST_ENVELOPE_UPDATED, { projectId });
    return {
      added, updated, unchanged, conflicted, total: draft.items.length
    };
  },
  ...
  upsertItem(projectId: string, item: Partial<ProjectCostItem> & { id?: string }): ProjectCostItem {
    ...
    this.saveDraft(projectId, draft => {
      ...
      if (!existing) {
        ...
        if ((item as { estimated?: CostSideData }).estimated) existing.estimated = (item as { estimated?: CostSideData }).estimated!;
        if ((item as { actual?: CostSideData }).actual) existing.actual = (item as { actual?: CostSideData }).actual!;
  ...
  allocatePurchaseToItem(params: {
    ...
    this.saveDraft(params.projectId, draft => {
      ...
      const now = new Date().toISOString();
      const link = item.procurement.links.find(l => l.purchaseOrderId === params.purchaseOrderId && l.breakdownItemId === params.breakdownItemId);
      if (link) {
    ...
    const ensuredTargetItem = ensureProjectCostItem(targetItem, 'Allocation did not update target item');
    bus.emit(APP_EVENTS.PURCHASE_ALLOCATED_TO_COST, { projectId: params.projectId, costItemId: ensuredTargetItem.id });
