...
export class CentralDataService {
  ...
  */
  public getPurchaseOrdersByProjectId(projectId: string): PurchaseOrder[] {
    ...
    return relations.map(r => this.purchaseOrderCache.get(r.purchaseOrderId))
  ...
  public updateTender(id: string, updates: Partial<Tender>): Tender | null {
    ...
    if (updates.status && updates.status !== existing.status) {
      console.log(`🔄 تم تغيير حالة المنافسة ${existing.name} من ${existing.status} إلى ${updates.status}`);
  ...
  public deleteProject(id: string): boolean {
    ...
    if (deleted) {
      ...
      this.projectPurchaseRelations = this.projectPurchaseRelations.filter(r => r.projectId !== id);
      this.saveRelations();
  ...
  const id = boqData.id ?? `boq_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const boq: BOQData = {
      ...
      totalValue: typeof boqData.totalValue === 'number' ? boqData.totalValue : (Array.isArray(boqData.items) ? boqData.items.reduce((s, it) => s + (it.totalPrice || 0), 0) : 0),
