    ...
    try {
      ...
      if (pricingData?.pricing) {
        ...
        for (const [itemId, itemPricing] of pricingEntries) {
          if (itemPricing) {
            // حساب الإجماليات
            const materialsTotal = this.sumResourceTotals(
              this.toResourceArray(itemPricing.materials),
            )
            const laborTotal = this.sumResourceTotals(this.toResourceArray(itemPricing.labor))
            const equipmentTotal = this.sumResourceTotals(
              this.toResourceArray(itemPricing.equipment),
            )
            const subcontractorsTotal = this.sumResourceTotals(
              this.toResourceArray(itemPricing.subcontractors),
            )
            ...
            result.set(itemId, {
              materials: this.toResourceArray(
              ...
              equipment: this.toResourceArray(
                itemPricing.equipment,
              ) as EstimatedPricingData['equipment'],
              subcontractors: this.toResourceArray(
