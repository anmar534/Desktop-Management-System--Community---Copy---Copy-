/**
 * 🧠 Pricing Engine - Centralized pricing computations & normalization
 * هدف الملف: توحيد جميع معادلات التسعير (subtotal + النسب الإضافية + breakdown)
...
export function enrichPricingItems(
  ...
  const isPlaceholderDescription = (desc: unknown): boolean => {
    ...
    if (!s) return true
    // Arabic placeholders variants + generic 'غير محدد'
  ...
  const deriveBetterDescription = (
    ...
    return cand && !isPlaceholderDescription(cand) ? String(cand) : `البند ${fallbackIdx}`
  }
