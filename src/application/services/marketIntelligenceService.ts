...
class MarketIntelligenceServiceImpl implements MarketIntelligenceService {
  ...
  private translateTrend(trend: string): string {
    switch (trend) {
      case 'increasing': return '┘à╪ز╪▓╪د┘è╪» / Increasing'
      case 'decreasing': return '┘à╪ز┘╪د┘é╪╡ / Decreasing'
      case 'stable': return '┘à╪│╪ز┘é╪▒ / Stable'
  ...
  private translateSentiment(sentiment: string): string {
    switch (sentiment) {
      case 'positive': return '╪ح┘è╪ش╪د╪ذ┘è / Positive'
      case 'negative': return '╪│┘╪ذ┘è / Negative'
      case 'neutral': return '┘à╪ص╪د┘è╪» / Neutral'
  ...
  private translateCompetition(level: string): string {
    switch (level) {
      case 'high': return '╪╣╪د┘┘è / High'
      case 'medium': return '┘à╪ز┘ê╪│╪╖ / Medium'
      case 'low': return '┘à┘╪«┘╪╢ / Low'
