...
export const costVarianceService = {
  ...
  analyzeProject(projectId: string, opts: { force?: boolean } = {}) {
    ...
    if (!env?.draft)
      return {
        alerts: [],
        stats: { itemsAnalyzed: 0, triggered: 0, projectVariancePct: 0, projectVarianceValue: 0 },
      }
    ...
    if (!config.enabled)
      return {
        ...
        stats: {
          ...
          triggered: 0,
          projectVariancePct: draft.totals.variancePct,
          projectVarianceValue: draft.totals.varianceTotal,
        },
      }
    ...
    for (const item of draft.items) {
      ...
      const exceedsPct =
        itemPctLimit !== undefined &&
        variancePct !== undefined &&
        Math.abs(variancePct) >= itemPctLimit
      const exceedsValue =
        itemValueLimit !== undefined &&
        varianceValue !== undefined &&
        Math.abs(varianceValue) >= itemValueLimit
    ...
    if (
      thresholds.projectVariancePct !== undefined &&
      Math.abs(projectVariancePct) >= thresholds.projectVariancePct
    ) {
      alerts.push({
        id: createAlertId(),
        projectId,
    ...
    if (
      erosionPct !== undefined &&
      thresholds.erosionPct !== undefined &&
      Math.abs(erosionPct) >= thresholds.erosionPct
    ) {
      alerts.push({
        id: createAlertId(),
        projectId,
