...
class DecisionSupportServiceImpl implements DecisionSupportService {
  ...
  private identifyKeyFactors(scenario: DecisionScenario, framework: BidNoBidFramework, categoryScores: any) {
    ...
    Object.entries(categoryScores).forEach(([category, score]) => {
      if (score >= 70) {
        positive.push(`┘é┘ê╪ر ┘┘è ${this.translateCategory(category)} / Strong ${category} performance`)
      } else if (score <= 30) {
        negative.push(`╪╢╪╣┘ ┘┘è ${this.translateCategory(category)} / Weak ${category} performance`)
      } else {
        neutral.push(`╪ث╪»╪د╪ة ┘à╪ز┘ê╪│╪╖ ┘┘è ${this.translateCategory(category)} / Average ${category} performance`)
      }
    })
  ...
  private identifyCriticalIssues(scenario: DecisionScenario, framework: BidNoBidFramework, categoryScores: any): string[] {
    ...
    if (categoryScores.financial < 40) {
      issues.push('┘à╪«╪د┘ê┘ ┘à╪د┘┘è╪ر ┘â╪ذ┘è╪▒╪ر ╪ز╪ز╪╖┘╪ذ ┘à╪▒╪د╪ش╪╣╪ر / Significant financial concerns requiring review')
    }

    if (categoryScores.risk < 30) {
      issues.push('┘à╪│╪ز┘ê┘ë ┘à╪«╪د╪╖╪▒ ╪╣╪د┘┘è ┘è╪ز╪╖┘╪ذ ╪د╪│╪ز╪▒╪د╪ز┘è╪ش┘è╪ر ╪ز╪«┘┘è┘ / High risk level requiring mitigation strategy')
    }

    if (categoryScores.operational < 35) {
      issues.push('╪ز╪ص╪»┘è╪د╪ز ╪ز╪┤╪║┘è┘┘è╪ر ┘é╪» ╪ز╪ج╪س╪▒ ╪╣┘┘ë ╪د┘╪ز┘┘┘è╪░ / Operational challenges may impact execution')
    }
  ...
  private identifyOpportunities(scenario: DecisionScenario, framework: BidNoBidFramework, categoryScores: any): string[] {
    ...
    if (categoryScores.market > 70) {
      opportunities.push('┘╪▒╪╡╪ر ╪│┘ê┘é┘è╪ر ┘à┘à╪ز╪د╪▓╪ر ┘┘┘┘à┘ê / Excellent market opportunity for growth')
    }

    if (categoryScores.strategic > 75) {
      opportunities.push('╪ز┘ê╪د┘┘é ╪د╪│╪ز╪▒╪د╪ز┘è╪ش┘è ┘é┘ê┘è ┘à╪╣ ╪ث┘ç╪»╪د┘ ╪د┘╪┤╪▒┘â╪ر / Strong strategic alignment with company goals')
    }

    if (categoryScores.financial > 80) {
      opportunities.push('╪╣╪د╪خ╪» ┘à╪د┘┘è ┘à╪ش╪▓┘è ┘à╪ز┘ê┘é╪╣ / Attractive financial returns expected')
    }
  ...
  private identifyThreats(scenario: DecisionScenario, framework: BidNoBidFramework, categoryScores: any): string[] {
    ...
    if (categoryScores.risk < 35) {
      threats.push('┘à╪«╪د╪╖╪▒ ╪╣╪د┘┘è╪ر ┘é╪» ╪ز╪ج╪س╪▒ ╪╣┘┘ë ╪د┘┘╪ش╪د╪ص / High risks may impact success')
    }
  ...
  private generateComparisonRecommendations(scenarios: DecisionScenario[], matrix: ComparisonMatrix): string[] {
    ...
    recommendations.push(`┘è┘┘╪╡╪ص ╪ذ╪د┘╪ز╪▒┘â┘è╪▓ ╪╣┘┘ë ╪د┘╪│┘è┘╪د╪▒┘è┘ê ╪د┘╪ث╪╣┘┘ë ╪ز┘é┘è┘è┘à╪د┘ï (╪د┘┘à╪▒╪ز╪ذ╪ر ${topScenario.rank})`)

    if (matrix.rankings.length > 2) {
      recommendations.push('┘à╪▒╪د╪ش╪╣╪ر ╪د┘╪│┘è┘╪د╪▒┘è┘ê┘ç╪د╪ز ┘à┘╪«┘╪╢╪ر ╪د┘╪ث╪»╪د╪ة ┘╪ز╪ص╪»┘è╪» ┘╪▒╪╡ ╪د┘╪ز╪ص╪│┘è┘')
    }

    recommendations.push('╪ح╪ش╪▒╪د╪ة ╪ز╪ص┘┘è┘ ╪ص╪│╪د╪│┘è╪ر ┘┘╪╣┘ê╪د┘à┘ ╪د┘╪ص╪▒╪ش╪ر ┘┘è ╪د┘╪│┘è┘╪د╪▒┘è┘ê┘ç╪د╪ز ╪د┘┘à╪«╪ز╪د╪▒╪ر')
  ...
  async validateFramework(framework: BidNoBidFramework): Promise<ValidationResult> {
    ...
    if (Math.abs(totalWeight - 100) > 0.01) {
      errors.push(`┘à╪ش┘à┘ê╪╣ ╪د┘╪ث┘ê╪▓╪د┘ ┘è╪ش╪ذ ╪ث┘ ┘è╪│╪د┘ê┘è 100% (╪د┘╪ص╪د┘┘è: ${totalWeight}%)`)
    }
    ...
    if (framework.criteria.length < 5) {
      warnings.push('┘è┘┘╪╡╪ص ╪ذ╪ح╪╢╪د┘╪ر ╪د┘┘à╪▓┘è╪» ┘à┘ ╪د┘┘à╪╣╪د┘è┘è╪▒ ┘┘╪ص╪╡┘ê┘ ╪╣┘┘ë ╪ز┘é┘è┘è┘à ╪┤╪د┘à┘')
    }
  ...
  async importScenario(data: string, format: 'json' | 'csv'): Promise<DecisionScenario> {
    try {
      ...
      const newScenario = await this.createScenario({
        ...
        description: scenarioData.description || '╪│┘è┘╪د╪▒┘è┘ê ╪ز┘à ╪د╪│╪ز┘è╪▒╪د╪»┘ç ┘à┘ ┘à┘┘ ╪«╪د╪▒╪ش┘è',
