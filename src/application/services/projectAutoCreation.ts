...
export class EnhancedProjectAutoCreationService {
  ...
  private static async copyPricingData(tenderId: string, projectId: string): Promise<void> {
    try {
      console.log(`🔍 البحث عن بيانات التسعير للمنافسة: ${tenderId}`);

      const boqRepository = getBOQRepository();

      // الحصول على BOQ موجود إن وجد
      let boqData = await boqRepository.getByTenderId(tenderId);
      console.log(`📊 BOQ من النظام المركزي: ${boqData ? 'موجود' : 'غير موجود'}`);

      if (!boqData) {
        console.log('🔄 محاولة قراءة من pricingService (تطبيع مركزي)...');
        ...
        if (pricingArray && pricingArray.length > 0) {
          ...
          if (boqItems.length > 0) {
            ...
            boqData = newBoqData;
            console.log('✅ تم إنشاء BOQ من بيانات التسعير (باستخدام normalizePricing)');
          }
        }
      }
      ...
      if (boqData) {
        ...
        console.log('✅ تم نسخ بيانات التسعير إلى المشروع مع الهيكل المحسن (estimated/actual)');
      } else {
        console.warn('⚠️ لا توجد بيانات تسعير لنسخها');
      }
    } catch (error) {
