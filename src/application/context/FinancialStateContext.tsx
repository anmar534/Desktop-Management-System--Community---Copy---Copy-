...
import {
  useFinancialReports,
  type UseFinancialReportsReturn,
...
export function FinancialStateProvider({ children }: { children: ReactNode }) {
  ...
  const highlights = useMemo<FinancialHighlights>(
    () =>
      selectFinancialHighlights({
        invoices,
        budgets,
  ...
  const projectsValue = useMemo<UseProjectsReturn>(() => {
    console.log(
      '🔄 [FinancialStateContext] Creating projectsValue memo with projects:',
      projects.length,
    )
    return {
      projects,
  ...
  const value = useMemo<FinancialStateContextValue>(() => {
    console.log('🏭 [FinancialStateContext] Creating context value with:', {
      projectsValueProjectsCount: projectsValue.projects.length,
      invoicesCount: invoicesValue.invoices.length,
    })
    return {
