...
export interface UseDomainPricingEngineParams<
  TEntry extends RawAuthoringEntry = RawAuthoringEntry,
> {
...
export function transformAuthoringToDomainUIItems<TEntry extends RawAuthoringEntry>(options: {
  tenderId: string
  quantityItems: UseDomainPricingEngineParams<TEntry>['quantityItems']
  pricingMap: Map<string, TEntry>
  defaults: { administrative: number; operational: number; profit: number }
}): { items: DomainPricingItem[]; totalValue: number } {
  ...
  const componentInputs = Array.from(pricingMap.entries()).map(([id, data]) => {
    ...
    const adminAmount =
      baseCost * ((d.additionalPercentages?.administrative ?? defaults.administrative) / 100)
    const operationalAmount =
      baseCost * ((d.additionalPercentages?.operational ?? defaults.operational) / 100)
...
export function useDomainPricingEngine<TEntry extends RawAuthoringEntry>(
  params: UseDomainPricingEngineParams<TEntry>,
): UseDomainPricingEngineResult {
