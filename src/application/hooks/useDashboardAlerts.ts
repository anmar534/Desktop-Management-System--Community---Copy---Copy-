...
export function useDashboardAlerts(options: UseDashboardAlertsOptions = {}): UseDashboardAlertsResult {
  const {
    autoRefresh = false,
  ...
  const nextAlerts = buildAlerts(metrics, maxAlerts, readStateRef.current)
    ...
    setAlerts(nextAlerts.map((alert) => ({ ...alert, isRead: readStateRef.current.get(alert.id) ?? false })))
