...
export const useFinancialData = (): UseFinancialDataReturn => {
  ...
  const getProjectActualCost = useCallback(
    (projectId: string): number => {
      const projectExpensesData = expenses.filter((expense) => expense.projectId === projectId)
  ...
  const calculateFinancialData = useMemo((): FinancialData => {
    ...
    const recentExpenses = expenses.filter(
      (expense) => new Date(expense.createdAt) >= threeMonthsAgo,
    )
    ...
    const netProfit = grossProfit * 0.8 // خصم 20% للضرائب والمخصصات
    ...
    const costEfficiency =
      totalRevenue > 0 ? Math.max(0, (1 - totalExpenses / totalRevenue) * 100) : 0
