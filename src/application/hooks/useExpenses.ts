...
export const useExpenses = (): UseExpensesReturn => {
  ...
  // تحديث مصروف
  const updateExpense = useCallback(async (id: string, updatedExpense: Expense): Promise<boolean> => {
    try {
      const database = getExpenseDatabase();
      if (database) {
  ...
  const getExpensesByType = useCallback((isAdministrative: boolean): Expense[] => {
    return expenses.filter(expense => expense.isAdministrative === isAdministrative);
  }, [expenses]);

  // فلترة المصروفات حسب المشروع
  const getExpensesByProject = useCallback((projectId: string): Expense[] => {
