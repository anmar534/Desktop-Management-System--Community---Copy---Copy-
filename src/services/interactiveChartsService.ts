...
export class InteractiveChartsService {
   ...
   */
  async createChartConfiguration(config: Omit<ChartConfiguration, 'id' | 'createdAt' | 'updatedAt'>): Promise<ChartConfiguration> {
   ...
   */
  async updateChartConfiguration(chartId: string, updates: Partial<ChartConfiguration>): Promise<ChartConfiguration | null> {
    const configurations = await this.getChartConfigurations();
   ...
   */
  async getFinancialChartData(chartType: string, filters?: ChartFilter[]): Promise<ChartDataPoint[]> {
   ...
   */
  async getTimeSeriesData(dataSource: string, period: 'daily' | 'weekly' | 'monthly' | 'yearly'): Promise<TimeSeriesDataPoint[]> {
   ...
   * إزالة مستمع للأحداث
   */
  removeEventListener(chartId: string, listener: Function): void {
  ...
  private aggregateTendersBySuccessRate(tenders: any[]): ChartDataPoint[] {
    ...
    const pendingTenders = tenders.filter(t => t.status === 'قيد المراجعة' || t.status === 'pending').length;
  ...
  private aggregateProfitMarginByPeriod(financialData: any): ChartDataPoint[] {
    ...
    return Object.entries(monthlyData).map(([month, data]) => {
      const profitMargin = data.revenue > 0 ? ((data.revenue - data.expense) / data.revenue) * 100 : 0;
      return {
