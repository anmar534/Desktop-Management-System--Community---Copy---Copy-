...
import type { EnhancedKPICardProps, KPITrend } from '@/components/analytics/enhanced'
import {
  DollarSign,
  Building,
  FileText,
...
export class EnhancedKPIService {
  ...
  async getCriticalKPIs(): Promise<EnhancedKPICardProps[]> {
    const [totalRevenue, activeProjects, delayedProjects, safetyScore] = await Promise.all([
  ...
  async getSafetyKPIs(): Promise<EnhancedKPICardProps[]> {
    const [incidentRate, safetyTraining, complianceRate, inspectionScore, nearMissReports] =
      await Promise.all([
  ...
  private async calculateTotalRevenue(): Promise<{
    value: number
    trend: KPITrend
  ...
  private async calculateActiveProjects(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateDelayedProjects(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
    ...
    const delayedProjects = projects.filter((p) => {
      ...
      return endDate < today && p.status === 'active'
    }).length
  ...
  // المؤشرات المالية
  private async calculateMonthlyProfit(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateBudgetVariance(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateCollectionRate(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateOverdueReceivables(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateProfitMargin(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  // مؤشرات المشاريع
  private async calculateCompletionRate(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateQualityScore(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateCustomerSatisfaction(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateResourceUtilization(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateSchedulePerformance(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateCostPerformance(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  // مؤشرات السلامة
  private async calculateIncidentRate(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
    // محاكاة حساب معدل الحوادث
    const incidentRate = 1 as number // حادث واحد هذا الشهر
  ...
  private async calculateSafetyTraining(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateComplianceRate(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateInspectionScore(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
  ...
  private async calculateNearMissReports(): Promise<{
    value: number
    trend: KPITrend
    status: 'success' | 'warning' | 'danger' | 'info'
  }> {
    ...
    let status: 'success' | 'warning' | 'danger' | 'info' = 'info'
    if (nearMissReports >= 5 && nearMissReports <= 10)
      status = 'success' // مستوى جيد من الإبلاغ
    else if (nearMissReports < 5)
      status = 'warning' // قد يكون هناك نقص في الإبلاغ
