...
export class PredictiveAnalyticsService {
  ...
  async createPredictionModel(
    model: Omit<PredictionModel, 'id' | 'createdAt' | 'updatedAt'>,
  ...
  async generateForecastScenarios(
    type: 'revenue' | 'cashflow' | 'performance',
  ...
  private async predictSingleValue(
    ...
    switch (model.algorithm) {
      ...
      default:
        predictedValue =
          Object.values(features).reduce((sum, val) => sum + val, 0) / Object.keys(features).length
    }
  ...
  private async generateScenarioPredictions(
    scenarioType: ForecastScenario['type'],
    predictionType: string,
  ): Promise<PredictionResult[]> {
    // تعديل التنبؤات حسب نوع السيناريو
    const basePredictions = await this.predictRevenue(6)
    const multiplier =
      scenarioType === 'optimistic' ? 1.2 : scenarioType === 'pessimistic' ? 0.8 : 1.0
