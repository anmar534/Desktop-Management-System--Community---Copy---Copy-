...
export interface NotificationRule {
  ...
  type: 'project_critical' | 'deadline_warning' | 'budget_overrun' | 'opportunity' | 'risk' | 'performance' | 'custom';
...
export interface NotificationCondition {
  field: string;
  operator: 'equals' | 'not_equals' | 'greater_than' | 'less_than' | 'between' | 'contains' | 'starts_with' | 'ends_with';
  value: any;
...
export class SmartNotificationsService {
  ...
  // تقييم شروط القاعدة
  private async evaluateConditions(conditions: NotificationCondition[]): Promise<Record<string, any>[]> {
  ...
  private evaluateCondition(item: Record<string, any>, condition: NotificationCondition): boolean {
    ...
    switch (condition.operator) {
      ...
      case 'less_than':
        return Number(value) < Number(conditionValue);
      case 'between':
        return Number(value) >= Number(conditionValue[0]) && Number(value) <= Number(conditionValue[1]);
      case 'contains':
  ...
  private async createNotificationFromRule(rule: NotificationRule, data: Record<string, any>): Promise<SmartNotification> {
  ...
  // توليد رسالة الإشعار
  private generateNotificationMessage(rule: NotificationRule, data: Record<string, any>): string {
    return rule.description || 'تم تشغيل قاعدة إشعار';
  }

  // توليد رسالة الإشعار بالإنجليزية
  private generateNotificationMessageEn(rule: NotificationRule, data: Record<string, any>): string {
    return rule.descriptionEn || rule.description || 'Notification rule triggered';
  }

  // تنفيذ الإجراءات
  private async executeActions(actions: NotificationAction[], notification: SmartNotification): Promise<void> {
  ...
  // إرسال بريد إلكتروني
  private async sendEmail(notification: SmartNotification, config: Record<string, any>): Promise<void> {
    // تنفيذ إرسال البريد الإلكتروني
    console.log('Sending email for notification:', notification.title);
  }

  // إنشاء مهمة
  private async createTask(notification: SmartNotification, config: Record<string, any>): Promise<void> {
    // تنفيذ إنشاء مهمة
    console.log('Creating task for notification:', notification.title);
  }

  // تحديث الحالة
  private async updateStatus(notification: SmartNotification, config: Record<string, any>): Promise<void> {
    // تنفيذ تحديث الحالة
    console.log('Updating status for notification:', notification.title);
  }

  // تسجيل حدث
  private async logEvent(notification: SmartNotification, config: Record<string, any>): Promise<void> {
    // تنفيذ تسجيل الحدث
  ...
  // تحديث حالة الإشعار
  async updateNotificationStatus(notificationId: string, status: 'read' | 'dismissed' | 'archived'): Promise<boolean> {
