...
export interface EconomicIndicator {
  id: string
  indicatorType: 'inflation' | 'gdp' | 'construction_index' | 'currency' | 'interest_rate' | 'oil_price'
  name: string
  nameEn?: string
  currentValue: number
...
export interface MarketIntelligenceService {
  ...
  addMarketOpportunity(data: Omit<MarketOpportunity, 'id' | 'identifiedDate'>): Promise<MarketOpportunity>
  updateOpportunityStatus(id: string, status: MarketOpportunity['status']): Promise<MarketOpportunity>
  ...
  exportMarketData(type: 'materials' | 'labor' | 'indicators', format: 'json' | 'csv'): Promise<string>
...
class MarketIntelligenceServiceImpl implements MarketIntelligenceService {
  ...
  async updateEconomicIndicator(id: string, data: Partial<EconomicIndicator>): Promise<EconomicIndicator> {
    const indicators = await this.getEconomicIndicators()
  ...
  async addIndustryTrend(data: Omit<IndustryTrend, 'id' | 'publishedDate'>): Promise<IndustryTrend> {
    const trends = await this.getIndustryTrends()
  ...
  async getMarketOpportunities(filters?: MarketIntelligenceFilters): Promise<MarketOpportunity[]> {
    try {
      ...
      return opportunities.filter((opportunity: MarketOpportunity) => {
        ...
        if (filters.opportunityTypes && !filters.opportunityTypes.includes(opportunity.opportunityType)) {
          return false
        }
  ...
  async addMarketOpportunity(data: Omit<MarketOpportunity, 'id' | 'identifiedDate'>): Promise<MarketOpportunity> {
    const opportunities = await this.getMarketOpportunities()
  ...
  async updateOpportunityStatus(id: string, status: MarketOpportunity['status']): Promise<MarketOpportunity> {
    const opportunities = await this.getMarketOpportunities()
  ...
  async getMarketAnalysis(): Promise<MarketAnalysisResult> {
    try {
      ...
      const avgPriceChange = materials.length > 0
        ? materials.reduce((sum, m) => sum + m.priceChangePercent, 0) / materials.length
        : 0

      const totalLaborCategories = laborRates.length
      const avgRateChange = laborRates.length > 0
        ? laborRates.reduce((sum, r) => {
            const change = r.rateHistory.length > 1
              ? ((r.hourlyRate - r.rateHistory[r.rateHistory.length - 2].hourlyRate) / r.rateHistory[r.rateHistory.length - 2].hourlyRate) * 100
              : 0
            return sum + change
          }, 0) / laborRates.length
        : 0
      ...
      // Determine trends
      const materialCostTrend = avgPriceChange > 2 ? 'increasing' : avgPriceChange < -2 ? 'decreasing' : 'stable'
      const laborRateTrend = avgRateChange > 2 ? 'increasing' : avgRateChange < -2 ? 'decreasing' : 'stable'
      ...
      const competitionLevel = activeOpportunities > 10 ? 'high' : activeOpportunities > 5 ? 'medium' : 'low'
  ...
  async createAlert(alert: Omit<MarketAlert, 'id' | 'createdAt' | 'acknowledged'>): Promise<MarketAlert> {
    try {
      const alerts = await this.getMarketAlerts()
  ...
  async importMarketData(data: any, type: 'materials' | 'labor' | 'indicators'): Promise<boolean> {
    try {
      const storageKey = type === 'materials' ? this.MATERIALS_KEY
        : type === 'labor' ? this.LABOR_KEY
        : this.INDICATORS_KEY
  ...
  async exportMarketData(type: 'materials' | 'labor' | 'indicators', format: 'json' | 'csv'): Promise<string> {
    try {
      let data: any[]
  ...
  async generateMarketReport(filters?: MarketIntelligenceFilters): Promise<string> {
    try {
      ...
      let report = `# تقرير ذكاء السوق - Market Intelligence Report\n\n`
      report += `**تاريخ التقرير / Report Date:** ${reportDate}\n\n`

      // Executive Summary
      report += `## الملخص التنفيذي / Executive Summary\n\n`
      report += `- إجمالي المواد المتتبعة / Total Materials Tracked: ${analysis.summary.totalMaterials}\n`
      report += `- متوسط تغيير الأسعار / Average Price Change: ${analysis.summary.avgPriceChange}%\n`
      report += `- إجمالي فئات العمالة / Total Labor Categories: ${analysis.summary.totalLaborCategories}\n`
      report += `- متوسط تغيير الأجور / Average Rate Change: ${analysis.summary.avgRateChange}%\n`
      report += `- الفرص النشطة / Active Opportunities: ${analysis.summary.activeOpportunities}\n`
      report += `- التنبيهات الحرجة / Critical Alerts: ${analysis.summary.criticalAlerts}\n\n`

      // Market Trends
      report += `## اتجاهات السوق / Market Trends\n\n`
      report += `- اتجاه تكلفة المواد / Material Cost Trend: ${this.translateTrend(analysis.trends.materialCostTrend)}\n`
      report += `- اتجاه أجور العمالة / Labor Rate Trend: ${this.translateTrend(analysis.trends.laborRateTrend)}\n`
      report += `- معنويات السوق / Market Sentiment: ${this.translateSentiment(analysis.trends.marketSentiment)}\n`
      report += `- مستوى المنافسة / Competition Level: ${this.translateCompetition(analysis.trends.competitionLevel)}\n\n`

      // Top Materials by Price Change
      if (materials.length > 0) {
        report += `## أهم المواد حسب تغيير السعر / Top Materials by Price Change\n\n`
      ...
      if (laborRates.length > 0) {
        report += `## أعلى أجور العمالة / Highest Labor Rates\n\n`
        const topRates = laborRates
          .sort((a, b) => b.hourlyRate - a.hourlyRate)
          .slice(0, 5)

        topRates.forEach((rate, index) => {
          report += `${index + 1}. **${rate.skillCategory}** (${rate.skillLevel}) - ${rate.hourlyRate} ${rate.currency}/ساعة\n`
        })
        report += `\n`
      }
      ...
      if (opportunities.length > 0) {
        report += `## الفرص السوقية / Market Opportunities\n\n`
        const activeOpps = opportunities.filter(o => o.status === 'active').slice(0, 5)

        activeOpps.forEach((opp, index) => {
          report += `${index + 1}. **${opp.title}** - ${opp.estimatedValue.toLocaleString()} (احتمالية: ${opp.probability}%)\n`
        })
        report += `\n`
      }
      ...
      // Industry Trends
      if (trends.length > 0) {
        report += `## الاتجاهات الصناعية / Industry Trends\n\n`
      ...
      report += `*آخر تحليل: ${analysis.lastAnalyzed} / Last Analysis: ${analysis.lastAnalyzed}*`
  ...
  private translateTrend(trend: string): string {
    switch (trend) {
      case 'increasing': return 'متزايد / Increasing'
      case 'decreasing': return 'متناقص / Decreasing'
      case 'stable': return 'مستقر / Stable'
  ...
  private translateSentiment(sentiment: string): string {
    switch (sentiment) {
      case 'positive': return 'إيجابي / Positive'
      case 'negative': return 'سلبي / Negative'
