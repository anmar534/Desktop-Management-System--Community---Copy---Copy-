...
export class SaudiTaxService {
  ...
  async createVATTransaction(transactionData: Omit<VATTransaction, 'id' | 'createdAt' | 'updatedAt' | 'version'>): Promise<VATTransaction> {
  ...
  calculateVAT(amount: number, rate: number = this.VAT_RATE): { vatAmount: number; totalAmount: number } {
    const vatAmount = amount * rate
    const totalAmount = amount + vatAmount
    return { vatAmount, totalAmount }
  }
  
  calculateVATFromTotal(totalAmount: number, rate: number = this.VAT_RATE): { amount: number; vatAmount: number } {
  ...
  // حساب الزكاة
  async calculateZakat(year: string, assets: Omit<ZakatCalculation, 'id' | 'year' | 'totalZakatableAssets' | 'totalDeductibleLiabilities' | 'netZakatableAssets' | 'zakatRate' | 'zakatDue' | 'status' | 'createdAt' | 'updatedAt' | 'version'>): Promise<ZakatCalculation> {
    const totalZakatableAssets = assets.cash + assets.bankDeposits + assets.accountsReceivable + assets.inventory + assets.investments
  ...
  // مساعدات
  private getPeriodDates(period: string, periodType: 'monthly' | 'quarterly'): { startDate: string; endDate: string } {
...
<VATReturn>
  ...
  async validateVATReturn(returnId: string): Promise<{ isValid: boolean; errors: string[] }> {
    ...
    if (Math.abs(vatReturn.totalSales - (vatReturn.standardRatedSales + vatReturn.zeroRatedSales + vatReturn.exemptSales)) > 0.01) {
      errors.push('Total sales calculation error')
    }
    
    if (Math.abs(vatReturn.totalPurchases - (vatReturn.standardRatedPurchases + vatReturn.zeroRatedPurchases + vatReturn.exemptPurchases)) > 0.01) {
