...
class EnhancedProjectService {
  ...
  async getAllProjects(filters?: ProjectFilters, sort?: ProjectSortOptions): Promise<EnhancedProject[]> {
    try {
      if (!filters && !sort) {
  ...
  async validateProject(request: CreateProjectRequest | UpdateProjectRequest): Promise<ProjectValidationResult> {
    try {
  ...
  async linkProjectToTender(projectId: string, tenderId: string, linkType: string): Promise<TenderProjectLink> {
    try {
