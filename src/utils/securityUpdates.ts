...
export type SecurityUpdateLevel = 'current' | 'patch' | 'minor' | 'major' | 'unsupported' | 'unknown';
...
export const compareSemver = (a: string | undefined | null, b: string | undefined | null): number => {
...
const determineUpdateLevel = (current: SemverParts, latest: SemverParts | null, latestSameMajor: SemverParts | null): SecurityUpdateLevel => {
	if (!latest) {
		return 'unknown';
	}
...
const buildRecommendations = (
	...
	const releasesLagNote = releasesBehind > 0 ? ` (متأخر ${releasesBehind} إصدار${releasesBehind > 1 ? 'ات' : ''})` : '';
...
export const analyzeElectronReleases = (
	...
	if (!normalizedCurrent) {
		return {
			...
			riskLevel: 'unknown',
			needsAction: true,
			recommendedAction: 'تعذر تحليل رقم إصدار Electron الحالي. تأكد من أن package.json يحتوي على قيمة صالحة مثل "38.0.0".',
	...
	if (stableReleases.length === 0) {
		return {
			...
			riskLevel: 'unknown',
			needsAction: true,
			recommendedAction: 'لم يتم العثور على بيانات إصدارات Electron مستقرة. تحقق من الاتصال بالإنترنت أو المصدر المعتمد.',
	...
	const validReleaseInfo = releaseInfo.filter((item): item is { release: NormalizedRelease; semver: SemverParts } =>
		Boolean(item.semver)
	);
	...
	const latestSameMajorEntry = validReleaseInfo.find((item) => item.semver.major === currentSemver.major) ?? null;

	const releasesAhead = validReleaseInfo.filter((item) => compareSemver(item.semver.raw, currentSemver.raw) > 0);
	...
	const recommendedAction = buildRecommendations(updateLevel, latestSemver.raw, supportWindow, releasesAhead.length);
...
export const formatSecurityUpdateSummary = (report: ElectronSecurityReport): string => {
	...
	if (report.latestStableVersion) {
		lines.push(`- آخر إصدار مستقر متاح: ${report.latestStableVersion}${report.latestStableDate ? ` (صدر في ${report.latestStableDate})` : ''}`);
	} else {
		lines.push('- تعذر تحديد آخر إصدار مستقر.');
	}

	if (report.latestSameMajorVersion) {
		lines.push(`- أحدث إصدار في نفس الفرع: ${report.latestSameMajorVersion}${report.latestSameMajorDate ? ` (${report.latestSameMajorDate})` : ''}`);
	}
