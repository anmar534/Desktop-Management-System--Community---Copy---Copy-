...
class HistoricalComparisonService {
  ...
  async compareToBenchmark(
    ...
    try {
      ...
      const percentageDifference = benchmarkValue !== 0 ? (difference / benchmarkValue) * 100 : 0
  ...
  async generateYearOverYearReport(
    ...
    const metrics: ComparisonMetric[] = ['winRate', 'averageMargin', 'totalValue', 'bidCount']
    ...
    for (const metric of metrics) {
      ...
      const monthlyTrend = await this.compareMultiplePeriods(performances, metric, 'monthly', 12)
  ...
  private calculateMetricValue(performances: BidPerformance[], metric: ComparisonMetric): number {
  ...
  private generateComparisonInsights(
    ...
    if (direction === 'increase') {
      interpretation =
        metric === 'winRate' || metric === 'averageMargin' || metric === 'totalValue'
  ...
  private generateBenchmarkInsights(
    ...
    const analysis =
      `الأداء الحالي ${ratingArabic} مقارنة بـ${benchmarkTypeArabic}. ` +
  ...
  private getMetricLabel(metric: ComparisonMetric): string {
    switch (metric) {
      ...
      case 'bidCount':
        return 'عدد المناقصات'
      case 'averageBidValue':
        return 'متوسط قيمة المناقصة'
      case 'competitorCount':
        return 'عدد المنافسين'
      case 'preparationTime':
        return 'وقت التحضير'
      default:
        return metric
  ...
  private getBenchmarkTypeArabic(
    type: 'industry' | 'historical' | 'target' | 'competitor',
  ): string {
    switch (type) {
      case 'industry':
        return 'معيار الصناعة'
      case 'historical':
        return 'الأداء التاريخي'
      case 'target':
        return 'الهدف المحدد'
      case 'competitor':
        return 'أداء المنافسين'
    }
  }
  ...
  private generateMetricRecommendations(
    ...
    if (direction === 'decrease' && ['winRate', 'averageMargin', 'totalValue'].includes(metric)) {
      ...
      direction === 'increase' &&
      ['winRate', 'averageMargin', 'totalValue'].includes(metric)
    ) {
