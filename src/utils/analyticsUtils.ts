...
export function calculateROI(performances: BidPerformance[]): number {
  ...
  const totalReturn = wonBids.reduce((sum, p) => sum + (p.actualMargin || p.plannedMargin) * p.bidAmount / 100, 0)
 ...
 */
export function calculateCompetitiveIntensity(competitorCount: number, marketValue: number): number {
