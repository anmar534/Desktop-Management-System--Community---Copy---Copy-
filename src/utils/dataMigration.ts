...
class DataMigrationService {
   ...
   */
  async validateMigrationData(data: Record<string, unknown>[]): Promise<{
    isValid: boolean
    errors: string[]
    warnings: string[]
  }> {
  ...
  private inferOutcome(tender: Record<string, unknown>): 'won' | 'lost' | 'pending' | 'cancelled' {
    if (tender.status) {
  ...
  private inferPlannedMargin(tender: Record<string, unknown>): number {
  ...
  private inferRiskScore(tender: Record<string, unknown>): number {
