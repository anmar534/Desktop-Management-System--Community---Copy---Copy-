...
export function buildPricingMap(pricing: [string, unknown][] | Record<string, unknown> | undefined | null) {
 ...
 */
export function normalizePOItem(poItemInput: PurchaseOrderItemLike, poId: string, index: number): NormalizedPricingItem {
...
export function normalizeBOQItem(boqItemInput: BOQItem): NormalizedPricingItem {
  ...
  return normalizePricingItem(id, {
    ...
    additionalPercentages: toAdditionalPercentages((boqItem as UnknownRecord).additionalPercentages),
    __sourceType: 'boq',
    __raw: boqItem
  })
}

const getAdditionalPercentagesFromItem = (item: BOQItem | NormalizedPricingItem): AdditionalPercentages | undefined =>
...
const needsRepair = (item: BOQItem) => {
  ...
  const hasActual =
    item.actual?.quantity !== undefined &&
    item.actual.quantity > 0 &&
    item.actual.unitPrice > 0
...
const findMatchInPricingMap = (item: BOQItem, pricingMap: Map<string, NormalizedPricingItem>) => {
  ...
  for (const [, pricing] of pricingMap) {
    if (pricing.description && item.description && pricing.description.trim() === item.description.trim()) {
      return pricing
    }
  }
...
export interface RepairResult { updated: boolean; repairedItems: number; newItems: BOQItem[] }

export function repairBOQ(boq: BOQData, pricingMap: Map<string, NormalizedPricingItem>): RepairResult {
  ...
  const newItems = boq.items.map(item => {
    ...
    const patchedWithPercentages = patched as BOQItem & { additionalPercentages?: AdditionalPercentages }
    patchedWithPercentages.additionalPercentages = mergeAdditionalPercentages(
      activeValues.additionalPercentages,
      matchAdditionalPercentages
    ) ?? {}
