...
const sanitizeString = (value: unknown, maxLength: number, fallback = '', { allowEmpty = false } = {}) => {
...
const getDesktopBridge = () => {
  ...
  return (window as unknown as { electronAPI?: { desktop?: DesktopBridgeHandle } }).electronAPI?.desktop;
};

const isElectron = () => typeof window !== 'undefined' && Boolean(getDesktopBridge()?.secureAction);

const callDesktopBridge = async <TPayload, TResult>(action: DesktopSecureAction, payload: TPayload) => {
  if (!isElectron()) {
    return {
      allowed: true,
  ...
  try {
    ...
    if (rawResult && typeof rawResult === 'object') {
      ...
      const sanitizedPayload = (rawResult as { payload?: TResult }).payload ?? (payload as unknown as TResult);
...
const sanitizeNotificationRequest = (payload: NotificationRequest): NotificationRequest => {
  const severity: NotificationSeverity = ['info', 'success', 'warning', 'error', 'urgent', 'reminder'].includes(payload.severity)
    ? payload.severity
    : 'info';
  ...
  const description = sanitizeString(payload.description, NOTIFICATION_MAX_DESCRIPTION, '', { allowEmpty: true });
  const actionLabel = sanitizeString(payload.actionLabel, NOTIFICATION_MAX_ACTION, '', { allowEmpty: true });
...
const sanitizeDragRequest = (payload: DragAuthorizationRequest): DragAuthorizationRequest => {
  ...
  const sanitizedFiles = files.map((file, index) => {
    ...
    const size = Math.max(0, Math.min(Number.isFinite(file?.size) ? Number(file.size) : 0, DRAG_MAX_FILE_BYTES));
    totalBytes += size;
...
export const authorizeDragAndDrop = async (
  ...
  const result = await callDesktopBridge<DragAuthorizationRequest, DragAuthorizationResponse['payload']>(
    'drag-intent',
    sanitized
  );
...
export const authorizeExport = async (
  ...
  const result = await callDesktopBridge<ExportAuthorizationRequest, ExportAuthorizationRequest>('export', sanitized);
