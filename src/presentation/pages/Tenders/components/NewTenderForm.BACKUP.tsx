...
export function NewTenderForm({ onSave, onBack, existingTender }: NewTenderFormProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        console.debug(
          '[NewTenderForm][debug] Potential active portal layers:',
          potentialLayers.length,
          potentialLayers,
        )
        ...
        if (coveringLayers.length > 0) {
          console.warn(
            '[NewTenderForm][debug] Large overlay(s) detected that might block interaction:',
            coveringLayers,
          )
        }
      } catch (error) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const resolvedType =
        typeInput.length > 0 ? typeInput : (existingTender?.type ?? 'مناقصة عامة')
      ...
      const normalizedQuantities = quantities.map((row, index) => {
        ...
        const normalizedDescription =
          resolvedDescriptionText.length > 0 ? resolvedDescriptionText : ''
      ...
      console.log(
        '[NewTenderForm][handleSave] tenderData.quantities length:',
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground">
                يدعم: Excel (.xlsx, .xls), CSV, TSV, TXT (الأعمدة المتوقعة: رقم، وحدة، كمية،
                مواصفات)
              </p>
                      ...
                      <tr key={row.id}>
                        <td className="border p-2">
                          <Input
                            ...
                            onChange={(e) =>
                              handleQuantityChange(row.id, 'serialNumber', e.target.value)
                            }
                            placeholder={`${index + 1}`}
                            className="bg-background"
                          />
                        ...
                        <td className="border p-2">
                          <Input
                            value={row.quantity}
                            onChange={(e) =>
                              handleQuantityChange(row.id, 'quantity', e.target.value)
                            }
                            placeholder="الكمية"
                            type="number"
                            className="bg-background"
          ...
          <div className="flex justify-between items-center">
            {/* زر الإلغاء والعودة */}
            <Button type="button" variant="outline" size="lg" className="px-6" onClick={onBack}>
            ...
            <AlertDialog open={saveDialogOpen} onOpenChange={setSaveDialogOpen}>
              <AlertDialogTrigger asChild>
                <Button size="lg" className="px-8" disabled={!isFormValid || isLoading}>
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="space-y-3">
              {[
                ...
                {
                  item: 'جداول الكميات',
                  completed: quantities.some((q) => q.specifications),
                  required: false,
                },
