...
export function FinancialSummaryCard({ onSectionChange }: FinancialSummaryCardProps) {
  ...
  const cashOnHand =
    toNumber(data.totals.cashOnHand) || toNumber(data.cashflow.totals.endingBalance)
  const averageDailyOutflow = toNumber(data.cashflow.totals.averageDailyOutflow)
  const monthlyOutflowEstimate = Math.max(
    averageDailyOutflow * 30,
    toNumber(data.cashflow.totals.outflow),
  )
  ...
  const cashFlowStatus: 'healthy' | 'warning' | 'critical' =
    balanceRatio >= 100 ? 'healthy' : balanceRatio >= 70 ? 'warning' : 'critical'
      ...
      <CardContent className="p-3 space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-muted-foreground">
          <span data-testid="currency-base">
            العملة الأساسية: <span className="font-semibold text-foreground">{currencyBase}</span>
          </span>
          <span data-testid="currency-updated">أسعار الصرف: {currencyLastUpdatedLabel}</span>
        </div>
        ...
        <div className="space-y-1">
          {quickIndicators.map((indicator, index) => (
            ...
            >
              <div
                className={`flex items-center justify-between gap-2 rounded-lg border p-2 ${indicatorToneStyles[indicator.tone].container}`}
              >
                <div className="flex items-center gap-2">
                  <indicator.icon
                    className={`h-3 w-3 ${indicatorToneStyles[indicator.tone].accent}`}
                  />
        ...
        <div className="flex gap-1 border-t border-border pt-2">
          <Button size="sm" className="flex-1 text-xs" onClick={() => onSectionChange('financial')}>
