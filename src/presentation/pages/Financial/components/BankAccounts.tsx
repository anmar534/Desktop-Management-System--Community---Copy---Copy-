...
export function BankAccounts({ onSectionChange }: BankAccountsProps) {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    return accounts.filter((account) => {
      ...
      return (
        name.includes(term) || bank.includes(term) || number.includes(term) || iban.includes(term)
      )
    })
  }, [accounts, searchTerm])
  ...
  const AccountAnalysisCards = (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <DetailCard
        ...
        value={
          accounts.length > 0
            ? formatCurrency(bankAccountsData.overview.totalBalance / accounts.length)
            : formatCurrency(0)
        }
        subtitle="لكل حساب"
        icon={Wallet}
      ...
      <DetailCard
        ...
        value={
          accounts.length > 0
            ? formatCurrency(Math.max(...accounts.map((acc) => acc.currentBalance)))
            : formatCurrency(0)
        }
        subtitle="أعلى رصيد"
        icon={TrendingUp}
        ...
        <div className="lg:col-span-2 space-y-4">
          {isLoading && accounts.length === 0 ? (
            ...
            <EmptyState
              ...
              onAction={
                accounts.length === 0 ? () => onSectionChange('new-bank-account') : undefined
              }
            />
          ) : (
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-1 text-success">
                            ...
                            <span className="text-sm">
                              داخل: {formatCurrency(account.monthlyInflow)}
                            </span>
                          </div>
                          <div className="flex items-center gap-1 text-destructive">
                            <ArrowUpRight className="h-4 w-4" />
                            <span className="text-sm">
                              خارج: {formatCurrency(account.monthlyOutflow)}
                            </span>
                          </div>
                        </div>

                        <div
                          className={`flex items-center gap-1 ${netFlow >= 0 ? 'text-success' : 'text-destructive'}`}
                        >
                          {netFlow >= 0 ? (
