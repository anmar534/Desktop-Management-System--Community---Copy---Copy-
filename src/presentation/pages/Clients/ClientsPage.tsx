...
export function Clients({ onSectionChange }: ClientsProps) {
  ...
  const getClientProjects = (clientName: string): ClientProjectsSummary => {
    ...
    const clientProjects = projects.filter(
      (project: Project) => project.client?.toLowerCase() === normalizedName,
    )
  ...
  const getClientTotalValue = (clientName: string) => {
    ...
    return clientProjects.reduce((sum, project) => sum + (project.value ?? project.budget ?? 0), 0)
  }
  ...
  const ClientCard = ({ client, index }: ClientCardProps) => {
    ...
    const hasOutstandingPayments =
      Number.isFinite(outstandingPaymentsValue) && outstandingPaymentsValue > 0
    ...
    const paymentAlertTitle =
      client.paymentRating === 'poor' ? 'مخاطر في تحصيل المستحقات' : 'مدفوعات متأخرة'
    ...
    >
      ...
      {filteredClients.length === 0 && (
        <EmptyState
          ...
          title="لا توجد عملاء"
          description={
            searchTerm ? 'لم يتم العثور على عملاء مطابقين لبحثك' : 'لا توجد عملاء مسجلين حتى الآن'
          }
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {(() => {
                ...
                const clientProjectsData = client
                  ? getClientProjects(client.name)
