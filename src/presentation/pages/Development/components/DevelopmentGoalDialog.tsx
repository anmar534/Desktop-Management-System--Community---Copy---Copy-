...
export function DevelopmentGoalDialog({
  ...
  // Handle input change
  const handleChange = (field: keyof DevelopmentGoal, value: string | number) => {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-3">
              <Label className="text-right text-base font-semibold">القيم المستهدفة</Label>

              <div className="grid grid-cols-3 gap-3">
                {[2025, 2026, 2027].map((year) => (
                  <div key={year} className="grid gap-2">
                    <Label htmlFor={`target${year}`} className="text-right text-sm">
                      {year}
                    </Label>
                    <Input
                      id={`target${year}`}
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'جاري الحفظ...' : isEditMode ? 'حفظ التعديلات' : 'إضافة الهدف'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
