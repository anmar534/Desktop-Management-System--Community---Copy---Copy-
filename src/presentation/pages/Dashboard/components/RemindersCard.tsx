...
import { Calendar, Building2, Trophy, DollarSign, Plus, type LucideIcon } from 'lucide-react'
          ...
          {reminders.map((reminder, index) => (
            ...
            >
              ...
              >
                {/* الأيقونة والمحتوى */}
                <div
                  className={`p-1 rounded-md bg-muted ${reminderIconTone[reminder.color]} flex-shrink-0`}
                >
                  <reminder.icon className="h-3 w-3" />
                </div>

                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-medium text-foreground truncate">{reminder.title}</h4>
                ...
                <div
                  className={`text-center px-1.5 py-1 rounded border text-xs ${getTimeRemainingColor(reminder.timeRemaining)}`}
                >
                  <div className="font-bold leading-none">{reminder.timeRemaining.value}</div>
                  <div className="text-xs leading-none">{reminder.timeRemaining.unit}</div>
                </div>
              </div>
            </motion.div>
          ))}
