...
export function DashboardKPICards({ onSectionChange }: DashboardKPICardsProps) {
  ...
  const kpiCards = useMemo(() => {
    ...
    const targetKey = `targetValue${currentYear}` as
      | 'targetValue2025'
      | 'targetValue2026'
      | 'targetValue2027'
    ...
    const revenueProgress =
      revenueTarget > 0 ? Math.min((revenueActual / revenueTarget) * 100, 100) : 0
    const revenueStatus =
      revenueProgress >= 100
        ? 'exceeded'
        : revenueProgress >= 80
    ...
    const tendersProgress =
      tendersTarget > 0 ? Math.min((tendersActual / tendersTarget) * 100, 100) : 0
    const tendersStatus =
      tendersProgress >= 100
        ? 'exceeded'
        : tendersProgress >= 80
    ...
    const projectsProgress =
      projectsTarget > 0 ? Math.min((projectsActual / projectsTarget) * 100, 100) : 0
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={`font-semibold ${card.color}`}>{card.progress.toFixed(1)}%</span>
                </div>
                <Progress
                  value={card.progress}
                  className="h-2"
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      className={`font-medium ${
                        card.actual >= card.target ? 'text-success' : 'text-destructive'
                      }`}
                    >
