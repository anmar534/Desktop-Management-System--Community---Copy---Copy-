...
const IntegrationManager: React.FC<IntegrationManagerProps> = React.memo(({
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (integrationsData.length > 0) {
        ...
        setSyncOperations(allSyncOps.sort((a, b) => 
  ...
  const handleStartSync = useCallback(async (integrationId: string, syncType: SyncType = 'manual') => {
    try {
      setLoading(true)
  ...
  const handleToggleIntegration = useCallback(async (integrationId: string, currentStatus: IntegrationStatus) => {
    try {
      setLoading(true)
    ...
    <div className={`space-y-6 ${className}`} dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">إدارة الاتصالات مع الأنظمة الخارجية ومصادر البيانات</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>المتصلة:</span>
                      <span className="font-semibold text-green-600">{dataSummary.integrations.connected}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>النشطة:</span>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span>مكتملة:</span>
                      <span className="font-semibold text-green-600">{dataSummary.syncOperations.completed}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>فاشلة:</span>
                      <span className="font-semibold text-red-600">{dataSummary.syncOperations.failed}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>الإجمالي:</span>
            ...
            {filteredIntegrations.map((integration) => (
              <Card key={integration.id} className="hover:shadow-md transition-shadow">
                <CardHeader>
                  ...
                  {integration.nameEn && (
                    <CardDescription>{integration.nameEn}</CardDescription>
                  )}
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    <div className="text-sm text-gray-600">
                      <p>آخر مزامنة: {new Date(integration.lastSync).toLocaleDateString('ar-SA')}</p>
                      <p>المزامنة التلقائية: {integration.settings.autoSync ? 'مفعلة' : 'معطلة'}</p>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge variant={operation.status === 'completed' ? 'default' : 
                      ...
                      {operation.status === 'completed' ? 'مكتملة' :
                       operation.status === 'failed' ? 'فاشلة' :
                       operation.status === 'running' ? 'قيد التشغيل' : operation.status}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{operation.duration ? `${operation.duration}ث` : '-'}</p>
                    </div>
                  </div>
                  
                  {operation.status === 'running' && (
                    <Progress 
                      value={(operation.recordsProcessed / Math.max(operation.summary.totalRecords, 1)) * 100} 
                      className="mt-3"
                    />
                  )}
                </CardContent>
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mt-2">ستتوفر قريباً تقارير مفصلة عن أداء التكاملات</p>
          </div>
        </TabsContent>
      </Tabs>
    </div>
