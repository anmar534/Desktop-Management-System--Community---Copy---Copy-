...
const UnifiedSystemIntegration: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [modulesData, dataFlowsData, summaryData, conflictsData, updatesData] = await Promise.all([
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            مراقبة وإدارة التكامل بين جميع وحدات النظام
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{summary.activeModules}/{summary.totalModules}</div>
          ...
          >
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{summary.activeDataFlows}/{summary.totalDataFlows}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${getHealthColor(summary.systemHealth)}`}>
                  {summary.systemHealth === 'excellent' ? 'ممتازة' :
                   summary.systemHealth === 'good' ? 'جيدة' :
                   summary.systemHealth === 'warning' ? 'تحذير' : 'حرجة'}
                </div>
                <div className="flex items-center gap-2 mt-2">
                  {getSyncStatusIcon(summary.overallSyncStatus)}
                  <span className="text-xs text-muted-foreground">
                    {summary.overallSyncStatus === 'synced' ? 'متزامن' :
                     summary.overallSyncStatus === 'syncing' ? 'جاري المزامنة' : 'خطأ في المزامنة'}
                  </span>
                </div>
              </CardContent>
            </Card>
          ...
          >
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{Math.round(summary.dataConsistencyScore)}%</div>
                ...
                <Card className="hover:shadow-md transition-shadow">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm">
                        <span>الأخطاء:</span>
                        <span className={`font-medium ${module.errorCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {module.errorCount}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <Badge variant={module.status === 'active' ? 'default' : 'secondary'}>
                                    {module.status === 'active' ? 'نشط' : 
                                     module.status === 'inactive' ? 'غير نشط' : 'صيانة'}
                                  </Badge>
                                </div>
                              </div>
                            </div>
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-lg">
                        {modules.find(m => m.id === flow.sourceModule)?.name} → {modules.find(m => m.id === flow.targetModule)?.name}
                      </CardTitle>
                      <div className="flex items-center gap-2">
                        {getSyncStatusIcon(flow.status)}
                        <Badge variant={flow.status === 'active' ? 'default' : 'secondary'}>
                          {flow.status === 'active' ? 'نشط' :
                           flow.status === 'paused' ? 'متوقف' : 'خطأ'}
                        </Badge>
                      </div>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      نوع البيانات: {flow.dataType}
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <Badge variant="outline">
                          {flow.syncFrequency === 'realtime' ? 'فوري' :
                           flow.syncFrequency === 'hourly' ? 'كل ساعة' : 'يومي'}
                        </Badge>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-sm">
                        <span>الأخطاء:</span>
                        <span className={`font-medium ${flow.errorCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                          {flow.errorCount}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${
                          update.operation === 'create' ? 'bg-green-500' :
                          update.operation === 'update' ? 'bg-blue-500' : 'bg-red-500'
                        }`} />
                        <div>
                          <p className="text-sm font-medium">
                            {update.operation === 'create' ? 'إنشاء' :
                             update.operation === 'update' ? 'تحديث' : 'حذف'}
                            {' '}{update.dataType}
                          </p>
                          <p className="text-xs text-muted-foreground">
