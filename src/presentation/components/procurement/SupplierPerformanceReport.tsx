              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className={`p-2 rounded-lg ${status.color}`}>{status.icon}</div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-lg">
                      {metrics.length > 0
                        ? (
                            metrics.reduce((sum, m) => sum + m.onTimeDeliveryRate, 0) /
                            metrics.length
                          ).toFixed(1)
                        : 0}
                      %
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    {
                      label: 'ممتاز (90+)',
                      count: metrics.filter((m) => m.overallScore >= 90).length,
                      color: 'bg-green-500',
                    },
                    {
                      label: 'جيد جداً (80-89)',
                      count: metrics.filter((m) => m.overallScore >= 80 && m.overallScore < 90)
                        .length,
                      color: 'bg-blue-500',
                    },
                    {
                      label: 'جيد (70-79)',
                      count: metrics.filter((m) => m.overallScore >= 70 && m.overallScore < 80)
                        .length,
                      color: 'bg-yellow-500',
                    },
                    {
                      label: 'ضعيف (<70)',
                      count: metrics.filter((m) => m.overallScore < 70).length,
                      color: 'bg-red-500',
                    },
                  ].map((item) => (
        ...
        <TabsContent value="detailed" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>تقييم شامل لجميع الموردين مع المؤشرات الرئيسية</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      >
                        <TableCell>
                          <div>
                            <p className="font-medium">{supplier.supplierName}</p>
                            <Badge
                              className={getPerformanceColor(supplier.overallScore)}
                              variant="secondary"
                            >
                              {getPerformanceLabel(supplier.overallScore)}
                            </Badge>
        ...
        <TabsContent value="comparison" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>مقارنة أداء الموردين مع المعايير المرجعية</CardDescription>
            </CardHeader>
            <CardContent>
