...
export default function ProcurementIntegration({ className }: ProcurementIntegrationProps) {
  ...
  const [financialIntegration, setFinancialIntegration] = useState<FinancialIntegration | null>(null)
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-2xl font-bold ${getPerformanceColor(summary.performanceScore)}`}>
        ...
        <TabsContent value="projects" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                عرض حالة ربط المشتريات بالمشاريع وتتبع الميزانيات
              </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="text-center">
                                  ...
                                  <p className={`text-lg font-semibold ${
                                    selectedProject.remainingBudget >= 0 ? 'text-green-600' : 'text-red-600'
                                  }`}>
                                ...
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                  {selectedProject.procurementItems.map((item) => (
                                    <div key={item.id} className="flex justify-between items-center p-2 border rounded">
                                      ...
                                      <div className="text-left">
                                        <p className="font-semibold">{item.totalPrice.toLocaleString()} ريال</p>
                                        <Badge 
                                          variant={
                                            item.status === 'received' ? 'default' :
                                            item.status === 'ordered' ? 'secondary' :
                                            item.status === 'pending' ? 'outline' : 'destructive'
                                          }
                                        >
                                          {item.status === 'received' ? 'مستلم' :
                                           item.status === 'ordered' ? 'مطلوب' :
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between border-t pt-2">
                      ...
                      <span className={`font-semibold ${
                        financialIntegration.availableBudget >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={getBudgetVarianceColor(financialIntegration.budgetUtilization - 100)}>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${getPerformanceColor(summary.performanceScore)}`}>
                        {summary.performanceScore.toFixed(1)}%
                      </span>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${getBudgetVarianceColor(summary.budgetVariance)}`}>
