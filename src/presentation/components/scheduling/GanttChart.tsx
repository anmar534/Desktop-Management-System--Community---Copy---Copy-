...
export const GanttChart: React.FC<GanttChartProps> = ({
  ...
  const ganttTasks = useMemo(() => {
    ...
    return currentSchedule.tasks.map(
      (task): Task => ({
        start: task.start,
        end: task.end,
        name: task.name,
        ...
        isDisabled: false,
        styles: {
          backgroundColor: task.critical ? '#ef4444' : task.color || '#3b82f6',
      ...
      <Card className={className}>
        <CardContent className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <Button onClick={loadSchedule}>إنشاء جدولة جديدة</Button>
          </div>
        </CardContent>
      </Card>
    )
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">المدة: {currentSchedule.totalDuration} يوم</span>
            </div>

            <div className="flex items-center gap-2">
              <CheckCircle className="h-4 w-4 text-green-600" />
              <span className="text-sm">المهام: {ganttTasks.length}</span>
            </div>

            {criticalPath && (
              <div className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4 text-red-600" />
                <span className="text-sm">المسار الحرج: {criticalPath.path.length} مهمة</span>
              </div>
            )}

            <div className="flex items-center gap-2">
              <Badge variant="outline">الإصدار {currentSchedule.version}</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              return (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{task.end.toLocaleDateString('ar-SA')}</p>
                  </div>
                  <div>
