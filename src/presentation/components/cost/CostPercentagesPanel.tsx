...
export const CostPercentagesPanel: React.FC<CostPercentagesPanelProps> = ({
  ...
  return (
    <div
      className={'p-2 border border-border rounded-md bg-muted space-y-2 ' + (className ?? '')}
      dir="rtl"
    >
      <div className="grid grid-cols-3 gap-2">
        <div className="min-w-0">
          <span className="block text-xs text-muted-foreground">
            {labels?.administrative ?? 'الإدارية (%)'}
          </span>
          {commonInput('administrative', 'النسبة الإدارية')}
        </div>
        <div className="min-w-0">
          <span className="block text-xs text-muted-foreground">
            {labels?.operational ?? 'التشغيلية (%)'}
          </span>
      ...
      {onApplyAll && (
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs text-muted-foreground whitespace-nowrap">
            {labels?.applyAllHint ?? 'تُطبق على البنود الجديدة'}
          </span>
          <button
            onClick={onApplyAll}
            disabled={disabled}
