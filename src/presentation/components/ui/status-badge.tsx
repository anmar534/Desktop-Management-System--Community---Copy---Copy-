...
const statusBadgeVariants = cva('inline-flex items-center gap-1.5 font-medium transition-colors', {
  variants: {
    status: {
      ...
      // Financial Status
      underBudget:
        'bg-financial-underBudget/10 text-financial-underBudget border-financial-underBudget/30',
      ...
      overBudget:
        'bg-financial-overBudget/10 text-financial-overBudget border-financial-overBudget/30',
...
export interface ProjectStatusBadgeProps extends Omit<StatusBadgeProps, 'status'> {
  /**
   * تاريخ الاستحقاق
   */
...
export const ProjectStatusBadge = React.forwardRef<HTMLDivElement, ProjectStatusBadgeProps>(
...
export interface FinancialStatusBadgeProps extends Omit<StatusBadgeProps, 'status'> {
  /**
   * المبلغ المخطط
   */
  planned: number
...
export const FinancialStatusBadge = React.forwardRef<HTMLDivElement, FinancialStatusBadgeProps>(
  ({ planned, actual, warningThreshold = 0.9, ...props }, ref) => {
    ...
    return <StatusBadge ref={ref} status={status} value={getPercentage()} {...props} />
  },
)
