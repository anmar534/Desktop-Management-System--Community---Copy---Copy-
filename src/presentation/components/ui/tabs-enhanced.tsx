...
const tabsListVariants = cva('inline-flex items-center justify-start gap-1', {
  variants: {
    variant: {
...
export const EnhancedTabs = React.forwardRef<HTMLDivElement, EnhancedTabsProps>(
  (
    {
      tabs,
      defaultValue,
    ...
    return (
      ...
      >
        <TabsPrimitive.List
          className={cn(tabsListVariants({ variant, size }), alignmentClasses[align])}
        >
          {tabs.map((tab) => {
            const Icon = tab.icon
