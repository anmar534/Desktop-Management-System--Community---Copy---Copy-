  ...
  return (
    <div className="w-[500px] space-y-4 p-4 border rounded-lg">
      ...
      <div className="space-y-4">
        ...
        <Slider value={budget} onValueChange={setBudget} min={0} max={1000000} step={10000} />
  ...
  return (
    <div className="w-[400px] space-y-4 p-4 border rounded-lg">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div className={`text-xl font-bold ${getScoreColor(score[0])}`}>{score[0]}/10</div>
          <div className="text-xs text-muted-foreground">{getScoreLabel(score[0])}</div>
        </div>
      </div>
...
const PriorityLevelStory: React.FC = () => {
  ...
  const colors = ['text-success', 'text-warning', 'text-primary', 'text-destructive'] as const
  ...
  return (
    <div className="w-[400px] space-y-4 p-4 border rounded-lg">
      ...
      <div className="flex justify-between text-xs">
        {priorities.map((p, i) => (
          <span key={p} className={priority[0] === i ? colors[i] : 'text-muted-foreground'}>
            {p}
          </span>
        ))}
      </div>
  ...
  render: () => (
    <div className="max-w-4xl space-y-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground">مكون Slider لاختيار قيمة أو نطاق من المدى المحدد</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div className="p-3 bg-info/10 dark:bg-info/20 rounded">
            ...
            <p className="text-muted-foreground">قيمة واحدة - مثل مستوى الصوت أو التقدم</p>
            <code className="block mt-2 text-xs">{`defaultValue={[50]}`}</code>
          </div>
          <div className="p-3 bg-success/10 dark:bg-success/20 rounded">
            <h4 className="font-semibold mb-2">Range</h4>
            <p className="text-muted-foreground">نطاق - مثل السعر من...إلى</p>
            <code className="block mt-2 text-xs">{`defaultValue={[25, 75]}`}</code>
          </div>
        </div>
      </div>
