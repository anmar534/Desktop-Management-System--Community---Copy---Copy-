...
export const WithLabel: Story = {
  render: () => (
    <div className="w-[400px] space-y-2">
      ...
      <Textarea id="description" placeholder="أدخل وصف تفصيلي للمشروع..." />
    </div>
  ),
}
...
export const Disabled: Story = {
  render: () => (
    <div className="w-[400px] space-y-2">
      ...
      <Textarea id="disabled" placeholder="هذا الحقل معطل" disabled />
    </div>
  ),
}
...
export const WithError: Story = {
  render: () => (
    <div className="w-[400px] space-y-2">
      ...
      <Textarea id="error" placeholder="أدخل تعليقاتك..." aria-invalid="true" />
      <p className="text-sm text-destructive">يجب إدخال تعليقات بحد أدنى 10 أحرف</p>
    </div>
  ),
}
...
export const Required: Story = {
  render: () => (
    <div className="w-[400px] space-y-2">
      ...
      <Textarea id="required" placeholder="حقل إجباري..." required aria-required="true" />
    </div>
  ),
}
...
export const Readonly: Story = {
  render: () => (
    <div className="w-[400px] space-y-2">
      ...
      <Textarea id="readonly" value="هذا محتوى للقراءة فقط. لا يمكن تعديله." readOnly />
    </div>
  ),
}
    ...
    <div className="grid gap-6 w-[800px]">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Textarea placeholder="حقل به خطأ..." aria-invalid="true" />
        </div>

        <div className="space-y-2">
          <Label>معطل (Disabled)</Label>
          <Textarea placeholder="حقل معطل..." disabled />
        </div>

        <div className="space-y-2">
          <Label>إجباري (Required)</Label>
          <Textarea placeholder="حقل إجباري..." required />
        </div>

        <div className="space-y-2">
          <Label>للقراءة فقط (Readonly)</Label>
          <Textarea value="للقراءة فقط" readOnly />
        </div>
      </div>
    </div>
  ),
  ...
  render: () => (
    <div className="space-y-4 w-[600px]">
      <div className="space-y-2">
        ...
        <Textarea placeholder="ارتفاع صغير للنصوص القصيرة..." className="min-h-16" />
      </div>

      <div className="space-y-2">
        <Label>متوسط (min-h-24)</Label>
        <Textarea placeholder="ارتفاع متوسط للوصف..." className="min-h-24" />
      </div>

      <div className="space-y-2">
        <Label>كبير (min-h-32)</Label>
        <Textarea placeholder="ارتفاع كبير للملاحظات الطويلة..." className="min-h-32" />
      </div>

      <div className="space-y-2">
        <Label>كبير جداً (min-h-48)</Label>
        <Textarea placeholder="ارتفاع كبير جداً للنصوص المطولة..." className="min-h-48" />
      </div>
    </div>
  ),
}
    ...
    <div className="space-y-6 w-[600px]">
      ...
      <div className="space-y-4 p-4 border rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <Textarea id="tender-terms" placeholder="أدخل شروط المناقصة..." className="min-h-32" />
        </div>

        <div className="space-y-2">
          <Label htmlFor="tender-notes">ملاحظات إضافية</Label>
          <Textarea id="tender-notes" placeholder="أي ملاحظات أخرى..." className="min-h-20" />
        </div>
      </div>
      ...
      <div className="space-y-4 p-4 border rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            required
          />
          <p className="text-sm text-muted-foreground">الحد الأدنى: 50 حرف، الحد الأقصى: 500 حرف</p>
        </div>
      </div>
    </div>
  ),
