...
export function DataGrid<TData, TValue = unknown>({
  ...
  const virtualRows: (VirtualItem | { index: number; key: string; size: number; start: number })[] =
      ...
      >
        <table className="w-full text-sm" role="presentation">
          ...
          <tbody
            ...
            className={cn(
              'relative',
              enableVirtualization && '[height:var(--virtual-body-height)]',
            )}
          >
            {showEmptyState ? (
              <tr>
                        ...
                        style={{
                          flex:
                            cell.column.getSize() !== 150
                              ...
                              : '1 1 0%',
                          minWidth:
                            cell.column.getSize() !== 150 ? `${cell.column.getSize()}px` : '100px',
                        }}
                      >
                        <CellRenderer cell={cell} onCellEdit={onCellEdit} />
                      </td>
                    ))}
                  </tr>
                )
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {enableSelection && (
            <>
              ...
              />
              <span>
                تم تحديد {table.getSelectedRowModel().rows.length} من{' '}
