...
import { PaymentsReceivablesService } from '@/application/services/paymentsReceivablesService'

const INVOICE_STATUS_VARIANTS: Record<
  Invoice['status'],
  { label: string; labelEn: string; badgeClass: string }
> = {
  draft: {
    label: 'مسودة',
    labelEn: 'Draft',
  ...
  overdue: {
    label: 'متأخرة',
    labelEn: 'Overdue',
    badgeClass: 'bg-destructive/10 text-destructive border-none',
  },
  cancelled: {
    label: 'ملغاة',
    labelEn: 'Cancelled',
...
const RECEIVABLE_AGING_VARIANTS: Record<
  Receivable['status'],
...
> = {
  ...
  overdue_120_plus: {
    label: '+90 يوم',
    badgeClass: 'bg-destructive text-destructive-foreground border-destructive/50',
  },
}
...
export const PaymentsReceivables: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [invoicesData, paymentsData, receivablesData, alertsData, metricsData, statsData] =
        await Promise.all([
                ...
                .map((alert) => (
                  ...
                  >
                    <div className="flex items-center">
                      <div
                        className={`ml-3 h-2 w-2 rounded-full ${ALERT_TYPE_INDICATORS[alert.type]}`}
                      />
                      <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium">
                        {formatCurrency(metrics.agingBreakdown.current)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">1-30 يوم</span>
                      <span className="font-medium">
                        {formatCurrency(metrics.agingBreakdown.overdue30)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">31-60 يوم</span>
                      <span className="font-medium">
                        {formatCurrency(metrics.agingBreakdown.overdue60)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">61-90 يوم</span>
                      <span className="font-medium">
                        {formatCurrency(metrics.agingBreakdown.overdue90)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm">+90 يوم</span>
                      <span className="font-medium text-destructive">
                        {formatCurrency(metrics.agingBreakdown.overdue120Plus)}
                      </span>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="cursor-pointer hover:shadow-md transition-shadow">
              <CardContent className="p-6 text-center">
                ...
                <p className="mb-4 text-sm text-muted-foreground">
                  تحليل المستحقات حسب فترات العمر
                </p>
