...
export const ProfitabilityComparisonComponent: React.FC<ProfitabilityComparisonProps> = ({ className }) => {
  ...
  const loadAvailableItems = async () => {
    try {
      ...
      if (comparisonType === 'projects') {
        ...
        setAvailableItems(projects.map(p => ({
          id: p.projectId,
          name: p.projectName,
        ...
        setAvailableItems(clients.map(c => ({
          id: c.clientId,
          name: c.clientName,
  ...
  const radarData = comparison?.items.slice(0, 5).map(item => ({
  ...
  const barData = comparison?.items.map(item => ({
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={comparisonType} onValueChange={(value: 'projects' | 'clients') => setComparisonType(value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <div className="max-h-96 overflow-y-auto space-y-2 border rounded-md p-2">
                {filteredItems.map((item) => (
                  <div key={item.id} className="flex items-center space-x-2 space-x-reverse p-2 hover:bg-muted rounded">
                    <Checkbox
                      id={item.id}
              ...
              <div className="space-y-2 pt-4 border-t">
                ...
                <div className="text-sm space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{comparison.totalRevenue.toLocaleString('ar-SA')} ر.س</span>
                  </div>
                  <div className="flex justify-between">
                    <span>إجمالي الأرباح:</span>
                    <span className="font-medium text-green-600">{comparison.totalProfit.toLocaleString('ar-SA')} ر.س</span>
                  </div>
                  <div className="flex justify-between">
                    <span>متوسط الهامش:</span>
                    <span className="font-medium">{comparison.averageProfitMargin.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <BarChart data={barData}>
                      ...
                      <Tooltip 
                        formatter={(value, name) => [
                          ...
                          name === 'profit' ? 'الأرباح' : 
                          name === 'margin' ? 'هامش الربح' : 'العائد على الاستثمار'
                        ]}
                      />
                      <Legend />
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={400}>
                    <RadarChart data={radarData}>
                      ...
                      <PolarRadiusAxis />
                      <Radar name="الإيرادات" dataKey="revenue" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.1} />
                      <Radar name="الأرباح" dataKey="profit" stroke="#10b981" fill="#10b981" fillOpacity={0.1} />
                      <Radar name="هامش الربح" dataKey="margin" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.1} />
                      <Radar name="العائد على الاستثمار" dataKey="roi" stroke="#ef4444" fill="#ef4444" fillOpacity={0.1} />
                      <Legend />
                    </RadarChart>
                  </ResponsiveContainer>
                </CardContent>
                      ...
                      <tbody>
                        {comparison.items.map((item, index) => (
                          <tr key={item.id} className="border-b hover:bg-muted/50">
                            ...
                            <td className="p-2 text-green-600 font-medium">{item.profit.toLocaleString('ar-SA')} ر.س</td>
                            <td className="p-2">
