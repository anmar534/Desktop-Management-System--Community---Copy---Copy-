...
export const ProfitabilityAnalysis: React.FC<ProfitabilityAnalysisProps> = ({ className }) => {
  ...
  const averageProjectMargin = topProjects.length > 0 
  ...
  const averageClientMargin = topClients.length > 0 
  ...
  const profitabilityDistribution = [
    { name: 'مشاريع عالية الربحية', value: topProjects.filter(p => p.netProfitMargin >= 20).length, color: '#10b981' },
    { name: 'مشاريع متوسطة الربحية', value: topProjects.filter(p => p.netProfitMargin >= 10 && p.netProfitMargin < 20).length, color: '#f59e0b' },
    { name: 'مشاريع منخفضة الربحية', value: topProjects.filter(p => p.netProfitMargin < 10).length, color: '#ef4444' }
  ]
                  ...
                  <div key={project.id} className="border rounded-lg p-4">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-left">
                        <Badge variant={project.netProfitMargin >= 20 ? 'default' : project.netProfitMargin >= 10 ? 'secondary' : 'destructive'}>
                          #{index + 1}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                      <div>
                        ...
                        <p className="font-semibold">{project.totalRevenue.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">التكاليف</p>
                        <p className="font-semibold">{project.totalCosts.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">الربح الصافي</p>
                        <p className="font-semibold text-green-600">{project.netProfit.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{project.profitPerDay.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                  ...
                  <div key={client.id} className="border rounded-lg p-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {client.totalProjects} مشروع • الفئة: {
                            client.clientTier === 'platinum' ? 'بلاتينية' :
                            client.clientTier === 'gold' ? 'ذهبية' :
                            client.clientTier === 'silver' ? 'فضية' : 'برونزية'
                          }
                        </p>
                      </div>
                      <div className="text-left">
                        <Badge variant={
                          client.clientTier === 'platinum' ? 'default' :
                          client.clientTier === 'gold' ? 'secondary' :
                          client.clientTier === 'silver' ? 'outline' : 'destructive'
                        }>
                          #{index + 1}
                        </Badge>
                      </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
                      <div>
                        ...
                        <p className="font-semibold">{client.totalRevenue.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">إجمالي التكاليف</p>
                        <p className="font-semibold">{client.totalCosts.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">إجمالي الأرباح</p>
                        <p className="font-semibold text-green-600">{client.totalProfit.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{client.averageProjectValue.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">قيمة العميل مدى الحياة</p>
                        <p className="font-medium">{client.clientLifetimeValue.toLocaleString('ar-SA')} ر.س</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <Badge variant={
                          client.riskLevel === 'low' ? 'default' :
                          client.riskLevel === 'medium' ? 'secondary' : 'destructive'
                        }>
                          {client.riskLevel === 'low' ? 'منخفض' :
                           client.riskLevel === 'medium' ? 'متوسط' : 'عالي'}
                        </Badge>
                      </div>
                    </div>
                  </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={clientChartData}>
                    ...
                    <YAxis />
                    <Tooltip
                      formatter={(value) => [`${Number(value).toLocaleString('ar-SA')} ر.س`, 'الأرباح']}
                    />
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold text-blue-600">{averageProjectMargin.toFixed(1)}%</p>
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {topClients[0] ? `${topClients[0].totalProfit.toLocaleString('ar-SA')} ر.س` : ''}
                  </p>
                </div>
              </div>
            </CardContent>
