...
import { FinancialAnalyticsService } from '@/application/services/financialAnalyticsService'
import {
  LineChart,
  Line,
  BarChart,
...
export const FinancialAnalytics: React.FC = () => {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const monthlyData = [
        { month: 1, revenue: 1800000 },
        { month: 2, revenue: 1900000 },
        { month: 3, revenue: 2000000 },
        { month: 4, revenue: 2100000 },
        { month: 5, revenue: 2000000 },
        { month: 6, revenue: 2200000 },
        { month: 7, revenue: 2300000 },
        { month: 8, revenue: 2100000 },
        { month: 9, revenue: 2000000 },
        { month: 10, revenue: 2200000 },
        { month: 11, revenue: 2400000 },
        { month: 12, revenue: 2500000 },
      ]
      ...
      const sampleKPIs: FinancialKPI[] = [
        {
          ...
          trend: 'up',
          performance:
            sampleLiquidityData.currentRatio >= 2
              ? 'excellent'
              : sampleLiquidityData.currentRatio >= 1.5
                ? 'good'
                : sampleLiquidityData.currentRatio >= 1
                  ? 'average'
                  : 'poor',
        ...
        },
        {
          ...
          trend: 'up',
          performance:
            sampleProfitabilityData.netProfitMargin >= 15
              ? 'excellent'
              : sampleProfitabilityData.netProfitMargin >= 10
                ? 'good'
                : sampleProfitabilityData.netProfitMargin >= 5
                  ? 'average'
                  : 'poor',
          lastUpdated: new Date().toISOString(),
        },
            ...
            >
              ...
              <AlertDescription>
                ...
                <div className="text-sm text-gray-600 mt-1">
                  القيمة الحالية: {formatRatio(indicator.currentValue)} | الحد المطلوب:{' '}
                  {formatRatio(indicator.threshold)}
                </div>
              </AlertDescription>
            </Alert>
          ))}
              ...
              <Card key={kpi.id}>
                ...
                <CardContent>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    الهدف:{' '}
                    {kpi.unit === '%' ? formatPercentage(kpi.target) : formatRatio(kpi.target)}
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  مؤشرات قدرة الشركة على الوفاء بالتزاماتها قصيرة المدى
                </CardDescription>
              </CardHeader>
                ...
                {profitabilityRatios && (
                  <>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold">
                        {formatPercentage(profitabilityRatios.grossProfitMargin)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>هامش الربح الصافي</span>
                      <span className="font-bold">
                        {formatPercentage(profitabilityRatios.netProfitMargin)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>هامش التشغيل</span>
                      <span className="font-bold">
                        {formatPercentage(profitabilityRatios.operatingMargin)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>العائد على الأصول</span>
                      <span className="font-bold">
                        {formatPercentage(profitabilityRatios.returnOnAssets)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>العائد على حقوق الملكية</span>
                      <span className="font-bold">
                        {formatPercentage(profitabilityRatios.returnOnEquity)}
                      </span>
                    </div>
                  </>
                )}
                ...
                {activityRatios && (
                  <>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold">
                        {formatRatio(activityRatios.inventoryTurnover)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>معدل دوران المدينين</span>
                      <span className="font-bold">
                        {formatRatio(activityRatios.receivablesTurnover)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>متوسط فترة التحصيل</span>
                      <span className="font-bold">
                        {formatRatio(activityRatios.daysSalesOutstanding)} يوم
                      </span>
                    </div>
                  </>
                )}
                ...
                {leverageRatios && (
                  <>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold">
                        {formatPercentage(leverageRatios.debtToAssets)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>نسبة الدين إلى حقوق الملكية</span>
                      <span className="font-bold">
                        {formatPercentage(leverageRatios.debtToEquity)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>نسبة حقوق الملكية</span>
                      <span className="font-bold">
                        {formatPercentage(leverageRatios.equityRatio)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>تغطية الفوائد</span>
                      <span className="font-bold">
                        {formatRatio(leverageRatios.interestCoverage)}
                      </span>
                    </div>
                  </>
                )}
              ...
              <ResponsiveContainer width="100%" height={400}>
                <LineChart data={cashFlowForecast}>
                  ...
                  <Line
                    ...
                    stroke="#82ca9d"
                    name="التدفق الداخل المتوقع"
                  />
                  <Line
                    type="monotone"
                    dataKey="projectedOutflow"
                    stroke="#ff7300"
                    name="التدفق الخارج المتوقع"
                  />
                  <Line
                    type="monotone"
                    dataKey="netCashFlow"
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      cy="50%"
                      labelLine={false}
                      label={({ monthName, percent }) =>
                        `${monthName} ${(percent * 100).toFixed(0)}%`
                      }
                      outerRadius={80}
                      fill="#8884d8"
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span
                        className={`text-sm font-medium ${getPerformanceColor(kpi.performance)}`}
                      >
                        {kpi.performance === 'excellent'
                          ? 'ممتاز'
                          : kpi.performance === 'good'
