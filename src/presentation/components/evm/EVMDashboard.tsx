...
export const EVMDashboard: React.FC<EVMDashboardProps> = ({ projectId, className = '' }) => {
    ...
    try {
      ...
      const mockData: EVMDashboardData = {
        ...
        kpis: [
          ...
          { name: 'مؤشر أداء الجدولة', value: 0.9, target: 1.0, trend: 'down', status: 'warning' },
  ...
  const getHealthStatus = (metrics: EVMMetrics) => {
    ...
    } else if (metrics.costPerformanceIndex >= 0.8 || metrics.schedulePerformanceIndex >= 0.8) {
      return {
        status: 'warning',
