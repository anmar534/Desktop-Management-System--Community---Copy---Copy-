...
} from '@/application/services/projectReportingService'
import { projectReportingService } from '@/application/services/projectReportingService'
import type { KPIDashboard } from '@/application/services/kpiCalculationEngine'
import { kpiCalculationEngine } from '@/application/services/kpiCalculationEngine'
...
export const ProjectReports: React.FC<ProjectReportsProps> = ({ projectId, className = '' }) => {
  const [loading, setLoading] = useState(false)
  const [selectedReport, setSelectedReport] = useState<'status' | 'evm' | 'kpi' | 'dashboard'>(
    'status',
  )
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold">
                {formatPercentage(report.executiveSummary.completionPercentage)}
              </p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold">
                {formatPercentage(report.executiveSummary.budgetUtilization)}
              </p>
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatPercentage(report.performance.schedule.plannedProgress)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>التقدم الفعلي</span>
                <span className="font-medium">
                  {formatPercentage(report.performance.schedule.actualProgress)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>الانحراف</span>
                <span
                  className={`font-medium ${report.performance.schedule.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                >
                  {report.performance.schedule.variance >= 0 ? '+' : ''}
                  {formatPercentage(report.performance.schedule.variance)}
                </span>
              </div>
            </div>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatCurrency(report.performance.budget.totalBudget)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>المنفق حتى الآن</span>
                <span className="font-medium">
                  {formatCurrency(report.performance.budget.spentToDate)}
                </span>
              </div>
              <div className="flex justify-between">
                <span>المتبقي</span>
                <span className="font-medium">
                  {formatCurrency(report.performance.budget.remainingBudget)}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span
                  className={`font-medium ${
                    report.performance.budget.forecastAtCompletion <=
                    report.performance.budget.totalBudget
                      ? 'text-green-600'
                      : 'text-red-600'
                  }`}
                >
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatPercentage(report.resources.utilization)}
                </span>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-green-600">
              {data.projectsOverview.activeProjects}
            </p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-blue-600">
              {data.projectsOverview.completedProjects}
            </p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-orange-600">
              {data.projectsOverview.delayedProjects}
            </p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-green-600">
              {data.projectsOverview.onBudgetProjects}
            </p>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-red-600">
              {data.projectsOverview.overBudgetProjects}
            </p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold">
                {formatCurrency(data.financialSummary.totalPortfolioValue)}
              </p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-red-600">
                {formatCurrency(data.financialSummary.projectedOverrun)}
              </p>
  ...
  return (
    <div className={`space-y-6 ${className}`} dir="rtl">
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-4">
            <Select
              ...
              onValueChange={(value) => setSelectedReport(value as any)}
            >
      ...
      <div className="min-h-[600px]">
        {selectedReport === 'status' && statusReport && <StatusReportView report={statusReport} />}

        {selectedReport === 'dashboard' && dashboardData && <DashboardView data={dashboardData} />}
