      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge variant="outline">{filteredRisks.length} مخاطر</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            <div className="w-full sm:w-48">
              <Select
                ...
                onValueChange={(value) => setSelectedCategory(value as RiskCategory | 'all')}
              >
                <SelectTrigger>
                        ...
                        <Tooltip>
                            ...
                            >
                              <div className="text-center">
                                ...
                                <div className="text-xs">
                                  {getRiskLevelLabel(cell.riskLevel).ar}
                                </div>
                                {cell.risks.length > 0 && (
                                  <div className="text-xs mt-1">{cell.risks.length} مخاطر</div>
                                )}
                              </div>
                            </div>
                          </TooltipTrigger>
                          <TooltipContent side="top" className="max-w-xs">
                            <div className="space-y-2">
                              <div>
                                <strong>النقاط: {cell.riskScore}</strong> (
                                {getRiskLevelLabel(cell.riskLevel).ar})
                              </div>
                              <div>
                                الاحتمالية: {PROBABILITY_LABELS[cell.probability].ar} (
                                {cell.probability})
                              </div>
                              <div>
                              ...
                              {cell.risks.length > 0 && (
                                <div>
                                  ...
                                  {cell.risks.length > 3 && (
                                    <div className="text-xs">
                                      و {cell.risks.length - 3} مخاطر أخرى...
                                    </div>
                                  )}
                                </div>
                              )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(RISK_LEVEL_COLORS).map(([level, colorClass]) => {
              const count = filteredRisks.filter(
                (risk) => getRiskLevel(risk.riskScore) === level,
              ).length
              return (
