...
const getMarketPositionLabel = (position?: string | null) => {
  ...
  return (
    MARKET_POSITION_LABELS[position as keyof typeof MARKET_POSITION_LABELS] ||
    MARKET_POSITION_LABELS.unknown
  )
}
...
export const CompetitorTracker: React.FC<CompetitorTrackerProps> = React.memo(
  ({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'profile' | 'activities' | 'analysis'>(
      'overview',
    )
    ...
    const applyFilters = useCallback(() => {
      ...
      if (categoryFilter) {
        filtered = filtered.filter((competitor) => competitor.categories.includes(categoryFilter))
      }
    ...
    const handleCompetitorSelect = useCallback(
    ...
    const competitorStats = useMemo(() => {
      ...
      const highThreat = competitors.filter((c) =>
        ['high', 'critical'].includes((c.threatLevel || '') as string),
      ).length
      const totalMarketShare = competitors.reduce(
        (sum, c) => sum + (c.financialInfo?.marketShare || 0),
        0,
      )
        ...
        <div className="border-b border-border/60 p-6">
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
            {statCards.map((card) => {
              ...
              return (
                <div key={card.key} className={`rounded-lg p-4 shadow-sm ${variant.container}`}>
                ...
                {filteredCompetitors.map((competitor) => (
                  ...
                  >
                    <div className="mb-3 flex items-start justify-between">
                      ...
                      <Badge
                        variant="outline"
                        className={`text-xs ${getStatusBadgeClass(competitor.status)}`}
                      >
                    ...
                    <div className="space-y-2 text-sm text-muted-foreground">
                      ...
                      {competitor.financialInfo?.marketShare && (
                        <div>
                          الحصة السوقية: {formatPercentage(competitor.financialInfo.marketShare)}
                        </div>
                      )}
                    </div>
              ...
              <div className="rounded-lg border border-border bg-muted/40 p-6">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <div>
                    ...
                    <div className="space-y-2 text-sm text-muted-foreground">
                      <div>
                        <span className="font-medium text-foreground">النوع:</span>{' '}
                        {getCompetitorTypeLabel(selectedCompetitor.type)}
                      </div>
                      <div>
                        <span className="font-medium text-foreground">المنطقة:</span>{' '}
                        {selectedCompetitor.region}
                      </div>
                      <div>
                        <span className="font-medium text-foreground">الفئات:</span>{' '}
                        {selectedCompetitor.categories.join(', ')}
                      </div>
                      <div>
                        <span className="font-medium text-foreground">الحالة:</span>{' '}
                        {getStatusLabel(selectedCompetitor.status)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm text-muted-foreground">
                      {selectedCompetitor.financialInfo?.estimatedRevenue && (
                        <div>
                          <span className="font-medium text-foreground">الإيرادات المقدرة:</span>{' '}
                          {formatCurrency(selectedCompetitor.financialInfo.estimatedRevenue)}
                        </div>
                      )}
                      {selectedCompetitor.financialInfo?.marketShare && (
                        <div>
                          <span className="font-medium text-foreground">الحصة السوقية:</span>{' '}
                          {formatPercentage(selectedCompetitor.financialInfo.marketShare)}
                        </div>
                      )}
                      {selectedCompetitor.financialInfo?.employeeCount && (
                        <div>
                          <span className="font-medium text-foreground">عدد الموظفين:</span>{' '}
                          {selectedCompetitor.financialInfo.employeeCount}
                        </div>
                      )}
                    </div>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-1 text-sm text-muted-foreground">
                        ...
                        {activity.value && <div>القيمة: {formatCurrency(activity.value)}</div>}
                      </div>
                      <div className="text-xs text-muted-foreground">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          type: e.target.value as CompetitorFormData['type'],
                        }))
                      }
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-1"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({
                          ...prev,
                          status: e.target.value as CompetitorFormData['status'],
                        }))
                      }
                      className="w-full rounded-lg border border-border px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-1"
                    >
