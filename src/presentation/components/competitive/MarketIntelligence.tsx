...
export const MarketIntelligence: React.FC<MarketIntelligenceProps> = React.memo(
  ({ className = '', onDataUpdate, showAnalytics = true, compactMode = false }) => {
    // ===== STATE MANAGEMENT =====
    ...
    const filteredData = useMemo(() => {
      ...
      return {
        materials: materialCosts.filter(
          (m) =>
            !searchTerm ||
        ...
        indicators: economicIndicators.filter(
          (i) =>
            !searchTerm ||
            i.name.toLowerCase().includes(searchLower) ||
            i.nameEn?.toLowerCase().includes(searchLower),
        ),
        trends: industryTrends.filter(
          (t) =>
            !searchTerm ||
            t.title.toLowerCase().includes(searchLower) ||
            t.description.toLowerCase().includes(searchLower),
        ),
        opportunities: marketOpportunities.filter(
          (o) =>
            !searchTerm ||
    ...
    const handleExport = useCallback(
      async (type: 'materials' | 'labor' | 'indicators', format: 'json' | 'csv') => {
        try {
    ...
    const renderTrendBadge = (trend: string) => {
      const badgeClass =
        TREND_BADGE_VARIANTS[trend as keyof typeof TREND_BADGE_VARIANTS] || NEUTRAL_BADGE_CLASS
    ...
    const renderSentimentBadge = (sentiment: string) => {
      const badgeClass =
        SENTIMENT_BADGE_VARIANTS[sentiment as keyof typeof SENTIMENT_BADGE_VARIANTS] ||
        NEUTRAL_BADGE_CLASS
      ...
      <div className={`space-y-6 ${className}`} dir="rtl">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Badge
                    variant="outline"
                    className={COMPETITION_BADGE_VARIANTS[marketAnalysis.trends.competitionLevel]}
                  >
                    {marketAnalysis.trends.competitionLevel === 'high'
                      ? 'عالي'
                      : marketAnalysis.trends.competitionLevel === 'medium'
                        ? 'متوسط'
                        : 'منخفض'}
                  </Badge>
                </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {marketAnalysis.recommendations.map((recommendation, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-3 rounded-lg border border-info/40 bg-info/10 p-3"
                      >
                  ...
                  <div className="space-y-3">
                    {marketAnalysis.alerts.slice(0, 5).map((alert) => (
                      <div key={alert.id} className="flex items-start gap-3 p-3 border rounded-lg">
                        <div
                          className={`mt-2 h-3 w-3 rounded-full ${ALERT_SEVERITY_DOT_VARIANTS[alert.severity]}`}
                        />
          ...
          <TabsContent value="materials" className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold">
                تكاليف المواد ({filteredData.materials.length})
              </h3>
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-xs ${getPriceChangeBadgeClass(material.priceChangePercent)}`}
                        >
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {material.currentPrice} {material.currency}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${MATERIAL_AVAILABILITY_VARIANTS[material.availability]}`}
                          >
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-xs ${LABOR_SKILL_LEVEL_VARIANTS[rate.skillLevel]}`}
                        >
                          {rate.skillLevel === 'expert'
                            ? 'خبير'
                            : rate.skillLevel === 'senior'
                              ? 'أول'
                              : rate.skillLevel === 'intermediate'
                                ? 'متوسط'
                                : 'مبتدئ'}
                        </Badge>
                      </div>
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${LABOR_DEMAND_VARIANTS[rate.demand]}`}
                          >
                            {rate.demand === 'high'
                              ? 'عالي'
                              : rate.demand === 'medium'
                        ...
                        <div className="flex justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${LABOR_AVAILABILITY_VARIANTS[rate.availability]}`}
                          >
                            {rate.availability === 'abundant'
                              ? 'وفير'
                              : rate.availability === 'moderate'
                                ? 'متوسط'
                                : 'نادر'}
                          </Badge>
                        </div>
          ...
          <TabsContent value="indicators" className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold">
                المؤشرات الاقتصادية ({filteredData.indicators.length})
              </h3>
              <Button
                variant="outline"
                size="sm"
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-xs ${TREND_BADGE_VARIANTS[indicator.trend]}`}
                        >
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${INDICATOR_IMPACT_VARIANTS[indicator.impact]}`}
                          >
          ...
          <TabsContent value="opportunities" className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold">
                الفرص السوقية ({filteredData.opportunities.length})
              </h3>
            </div>
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-xs ${OPPORTUNITY_STATUS_VARIANTS[opportunity.status]}`}
                        >
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {opportunity.estimatedValue.toLocaleString()} ريال
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${OPPORTUNITY_COMPETITION_VARIANTS[opportunity.competitionLevel]}`}
                          >
                      ...
                      <div className="pt-2 border-t">
                        <p className="text-xs text-muted-foreground">
                          تم تحديدها:{' '}
                          {new Date(opportunity.identifiedDate).toLocaleDateString('ar-SA')}
                        </p>
                      </div>
                    </div>
                  </CardContent>
        ...
        {!compactMode && filteredData.trends.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>آخر الاتجاهات والتطورات في قطاع البناء والتشييد</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={trend.id} className="border rounded-lg p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        <Badge
                          variant="outline"
                          className={`text-xs ${TREND_IMPACT_VARIANTS[trend.impact]}`}
                        >
                          تأثير{' '}
                          {trend.impact === 'high'
                            ? 'عالي'
