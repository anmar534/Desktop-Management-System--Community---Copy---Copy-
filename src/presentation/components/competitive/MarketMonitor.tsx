...
const SELECT_BASE_CLASS =
  'rounded-lg border border-border px-3 py-2 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-1'
...
export const MarketMonitor: React.FC<MarketMonitorProps> = React.memo(
  ({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'opportunities' | 'trends' | 'alerts'>(
      'overview',
    )
    ...
    const marketMetrics = useMemo((): MarketMetrics => {
      ...
      const winRate =
        completedOpportunities.length > 0
          ? wonOpportunities.length / completedOpportunities.length
          : 0

      const totalCompetitors = filteredOpportunities.reduce(
        (sum, opp) => sum + opp.competitorCount,
        0,
      )
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
            <div className={`${PRIMARY_CARD_CLASS} p-4`}>
              <div className="text-2xl font-bold text-primary">
                {marketMetrics.totalOpportunities}
              </div>
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-foreground">
                        {formatCurrency(marketMetrics.averageValue)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>مستوى المنافسة:</span>
                      <span
                        className={`font-medium ${COMPETITION_CLASSES[marketMetrics.competitionLevel]}`}
                      >
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-foreground">
                        {formatPercentage(marketMetrics.marketShare)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className={`${MUTED_CARD_CLASS} p-4`}>
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    {uniqueRegions.slice(0, 5).map((region) => {
                      const regionOpportunities = filteredOpportunities.filter(
                        (opp) => opp.region === region,
                      )
                      const regionValue = regionOpportunities.reduce(
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
                  ...
                  {filteredOpportunities.length === 0 && (
                    <div
                      className={`${CARD_BASE_CLASS} p-6 text-center text-sm text-muted-foreground`}
                    >
                      ...
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-2">
                          ...
                          <div className="text-sm text-destructive/70">
                            القيمة: {formatCurrency(opportunity.estimatedValue)} • الموعد النهائي:{' '}
                            {new Date(opportunity.deadline).toLocaleDateString('ar-SA')}
                          </div>
                        </div>
