...
export const CompetitiveBenchmark: React.FC<CompetitiveBenchmarkProps> = React.memo(({
  ...
  const [selectedMetric, setSelectedMetric] = useState<keyof BenchmarkMetrics['metrics']>('winRate')
  const [sortBy, setSortBy] = useState<'ranking' | 'winRate' | 'marketShare' | 'profitMargin'>('ranking')
  ...
  const loadBenchmarkData = useCallback(async () => {
    ...
    try {
      ...
      const companyScore = (companyBenchmark.metrics.winRate + companyBenchmark.metrics.marketShare + companyBenchmark.metrics.profitMargin) / 3
      const betterCompetitors = benchmarks.filter(b => {
        const competitorScore = (b.metrics.winRate + b.metrics.marketShare + b.metrics.profitMargin) / 3
  ...
  // Handle sort change
  const handleSort = useCallback((metric: typeof sortBy) => {
    if (sortBy === metric) {
            ...
            <tbody>
              ...
              {companyMetrics && (
                <tr className="border-b border-gray-200 bg-blue-50">
                  ...
                  <td className="py-3 px-4">
                    <div>
                      <div className="font-medium text-blue-900">{companyMetrics.competitor.name}</div>
                  ...
                  <td className="py-3 px-4">
                    <span className={`flex items-center ${getTrendColor(companyMetrics.trend)}`}>
                      ...
                      <span className="ml-1 text-sm">
                        {companyMetrics.trend === 'improving' ? 'متحسن' :
                         companyMetrics.trend === 'declining' ? 'متراجع' : 'مستقر'}
                      </span>
                    </span>
                  </td>
                  {showDetailedMetrics && (
              ...
              {sortedBenchmarkData.map((benchmark) => (
                <tr key={benchmark.competitor.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4">{formatPercentage(benchmark.metrics.profitMargin)}</td>
                  <td className="py-3 px-4">
                    <span className={`flex items-center ${getTrendColor(benchmark.trend)}`}>
                      {getTrendIcon(benchmark.trend)}
                      <span className="ml-1 text-sm">
                        {benchmark.trend === 'improving' ? 'متحسن' :
                         benchmark.trend === 'declining' ? 'متراجع' : 'مستقر'}
                      </span>
                    </span>
                  </td>
                  {showDetailedMetrics && (
