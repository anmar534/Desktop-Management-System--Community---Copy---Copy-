...
import type { CompetitorData, MarketOpportunity } from '@/shared/types/competitive'
...
export const SWOTAnalysis: React.FC<SWOTAnalysisProps> = React.memo(
  ({
    ...
    const handleDeleteItem = useCallback(
      async (
        category: keyof Pick<SWOTData, 'strengths' | 'weaknesses' | 'opportunities' | 'threats'>,
        itemId: string,
      ) => {
        if (!swotData) return
    ...
    if (!swotData) {
      return (
        <div className={`bg-white rounded-lg shadow-sm border p-6 ${className}`}>
          <div className="text-center text-gray-500">لا توجد بيانات تحليل SWOT متاحة</div>
        </div>
      )
    }
                  ...
                  {swotData[activeQuadrant].map((item, index) => (
                    <div key={item.id} className="bg-white bg-opacity-50 rounded-lg p-4">
                      ...
                      <div className="flex items-center space-x-4 mb-3">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${getImpactColor(item.impact)}`}
                        >
                          التأثير:{' '}
                          {item.impact === 'high'
                            ? 'عالي'
                        ...
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${getImpactColor(item.urgency)}`}
                        >
                          الأولوية:{' '}
                          {item.urgency === 'high'
                            ? 'عالية'
                      ...
                      {item.metrics && (
                        <div className="mb-3">
                          <div className="text-sm">
                            <span className="font-medium">المقياس:</span> {item.metrics.value}{' '}
                            {item.metrics.unit}
                            <span
                              className={`ml-2 ${
                                item.metrics.trend === 'improving'
                                  ? 'text-green-600'
                                  : item.metrics.trend === 'declining'
                                    ? 'text-red-600'
                                    : 'text-gray-600'
                              }`}
                            >
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-lg">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">التأثير</label>
                    <select
                      value={newItem.impact}
                      onChange={(e) =>
                        setNewItem((prev) => ({ ...prev, impact: e.target.value as any }))
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewItem((prev) => ({ ...prev, urgency: e.target.value as any }))
                      }
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    >
