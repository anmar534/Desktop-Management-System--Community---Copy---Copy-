...
const BreadcrumbText = styled.span<{ active?: boolean }>`
  ...
  gap: ${designTokens.spacing[1]};
  color: ${(props) =>
    props.active ? designTokens.colors.text.primary : designTokens.colors.text.secondary};
  font-weight: ${(props) =>
    props.active
...
function collapseBreadcrumbs(items: BreadcrumbItem[], maxItems: number): BreadcrumbItem[] {
  ...
  return [first, { label: '...', labelAr: '...' }, ...remaining.slice(-(maxItems - 2)), last]
}
        ...
        {displayItems.map((item, index) => {
          ...
          return (
            <BreadcrumbItem key={index}>
              ...
              {!isLast && <Separator aria-hidden="true">{separator || defaultSeparator}</Separator>}
            </BreadcrumbItem>
          )
        })}
      </BreadcrumbList>
