...
import { Search, Bell, User, Settings, LogOut, Moon, Sun, Globe, ChevronDown } from 'lucide-react'
...
const IconButton = styled.button<{ active?: boolean }>`
  ...
  &:hover {
    background-color: ${({ active }: { active?: boolean }) =>
      active ? designTokens.colors.primary[100] : designTokens.colors.neutral[100]};
    color: ${({ active }: { active?: boolean }) =>
...
const UserAvatar = styled.div<{ src?: string }>`
  ...
  background-image: ${({ src }: { src?: string }) => (src ? `url(${src})` : 'none')};
...
const Dropdown = styled.div<{ show: boolean }>`
  ...
  transform: ${({ show }: { show: boolean }) => (show ? 'translateY(0)' : 'translateY(-10px)')};
      ...
      <NavSection>
        ...
        <UserMenuContainer ref={userMenuRef}>
          <UserButton
            ...
            aria-haspopup="true"
          >
            <UserAvatar src={userAvatar}>{!userAvatar && getUserInitials()}</UserAvatar>
