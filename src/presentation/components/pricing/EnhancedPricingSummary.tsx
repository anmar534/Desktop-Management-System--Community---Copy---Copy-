...
export const EnhancedPricingSummary: React.FC<EnhancedPricingSummaryProps> = ({
  ...
  const profitPercentage =
    totals.grandTotal > 0 ? (totals.totalProfit / totals.grandTotal) * 100 : 0
  const overheadPercentage =
    totals.grandTotal > 0
      ? ((totals.totalAdministrative + totals.totalOperational) / totals.grandTotal) * 100
      : 0
            ...
            <div className="pt-4 border-t space-y-2">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-orange-700 tabular-nums">
                  {formatCurrency(totals.totalEquipment)}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">المقاولون:</span>
                <span className="font-semibold text-green-700 tabular-nums">
                  {formatCurrency(totals.totalSubcontractors)}
                </span>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right flex-1 mr-4">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  ({overheadPercentage.toFixed(1)}% من الإجمالي)
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-blue-700 tabular-nums">
                  {formatCurrency(totals.totalAdministrative)}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground flex items-center gap-2">
                ...
                <span className="font-bold text-orange-700 tabular-nums">
                  {formatCurrency(totals.totalOperational)}
                </span>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right flex-1 mr-4">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  ({profitPercentage.toFixed(1)}% من الإجمالي)
                </p>
              </div>
            </div>
