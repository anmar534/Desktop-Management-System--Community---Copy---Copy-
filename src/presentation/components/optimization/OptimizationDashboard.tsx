...
import { systemIntegrationService } from '@/application/services/systemIntegrationService'

interface OptimizationDashboardProps {
  className?: string
}

export const OptimizationDashboard: React.FC<OptimizationDashboardProps> = ({ className = '' }) => {
    ...
    <div className={`p-6 space-y-6 ${className}`} dir="rtl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={handleOptimizePerformance} disabled={isOptimizing} variant="outline">
            {isOptimizing ? (
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <Progress
                      ...
                      // @ts-ignore
                      indicatorClassName={
                        performanceReport.metrics.errorRate > 5 ? 'bg-red-500' : 'bg-green-500'
                      }
                    />
                  </CardContent>
                </Card>
        ...
        <TabsContent value="integration" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>إدارة التكامل مع أنظمة المنافسات الخارجية</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {tenderSystems.map((system) => (
                    ...
                    >
                      <div className="flex items-center space-x-3 space-x-reverse">
                        <div
                          className={`w-3 h-3 rounded-full ${getStatusColor(system.syncStatus)}`}
                        />
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline">{getStatusText(system.syncStatus)}</Badge>
                        <Button
                          size="sm"
                          onClick={() => handleSyncSystem('tender', system.id)}
                          disabled={isSyncing || !system.isActive}
                        >
                          {isSyncing ? <RefreshCw className="h-4 w-4 animate-spin" /> : 'مزامنة'}
                        </Button>
                      </div>
                    </div>
                  ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>إدارة التكامل مع الأنظمة المالية والمحاسبية</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="history" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>تاريخ عمليات المزامنة مع الأنظمة الخارجية</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {syncHistory.map((entry, index) => (
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-xs text-gray-500">{entry.result.duration}ms</p>
                      </div>
                    </div>
                  ))}
                </div>
