...
export function AnalyticsProvider({ children, autoRefresh = true }: AnalyticsProviderProps) {
  ...
  const loadBidPerformances = useCallback(
    async (filter?: AnalyticsFilter) => {
      try {
        dispatch({ type: 'SET_LOADING', payload: true })
        const performances = await analyticsService.getAllBidPerformances({
          filters: filter || state.globalFilter,
        })
  ...
  // Add bid performance
  const addBidPerformance = useCallback(
    async (performance: Omit<BidPerformance, 'id' | 'createdAt' | 'updatedAt'>) => {
      try {
  ...
  const updateBidPerformance = useCallback(
    async (id: string, updates: Partial<BidPerformance>) => {
      try {
  ...
  const deleteBidPerformance = useCallback(
    async (id: string) => {
      try {
