...
import {
  ...
  type MarketTrendPrediction,
} from '@/shared/utils/ml/predictionModels'
import {
  optimizeBidAmount,
  analyzePriceSensitivity,
  type BidOptimization,
  type PriceSensitivityAnalysis,
  type OptimizationParameters,
} from '@/shared/utils/pricing/priceOptimization'
...
export const PredictiveAnalytics: React.FC<PredictiveAnalyticsProps> = React.memo(
  ({ filter, className = '' }) => {
    ...
    // Prediction state
    const [currentPrediction, setCurrentPrediction] = useState<WinProbabilityPrediction | null>(
      null,
    )
    const [currentOptimization, setCurrentOptimization] = useState<BidOptimization | null>(null)
    const [currentTrendPrediction, setCurrentTrendPrediction] =
      useState<MarketTrendPrediction | null>(null)
    ...
    const [predictionForm, setPredictionForm] =
      useState<PredictionFormState>(INITIAL_PREDICTION_FORM)
    ...
    const handlePredictionFormChange = useCallback(
      <K extends keyof PredictionFormState>(field: K, value: PredictionFormState[K]) => {
        setPredictionForm((prev) => ({ ...prev, [field]: value }) as PredictionFormState)
      },
      [],
    )

    const handleOptimizationParamsChange = useCallback(
      <K extends keyof OptimizationParameters>(field: K, value: OptimizationParameters[K]) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mt-6">
              ...
              <div className="bg-warning/10 rounded-lg p-4">
                <div className="text-2xl font-bold text-warning">
                  {predictionSummary.riskLevel === 'low'
                    ? 'منخفض'
                    : predictionSummary.riskLevel === 'medium'
                      ? 'متوسط'
                      : 'عالي'}
                </div>
              ...
              <div className="bg-info/10 rounded-lg p-4">
                <div className="text-2xl font-bold text-info">{predictionSummary.confidence}%</div>
          ...
          {activeTab === 'predictions' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label
                    htmlFor="prediction-estimated-value"
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handlePredictionFormChange('estimatedValue', Number(e.target.value))
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label
                    htmlFor="prediction-competitor-count"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    عدد المنافسين
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      handlePredictionFormChange('competitorCount', Number(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                  />
                </div>
                <div>
                  <label
                    htmlFor="prediction-client-type"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    نوع العميل
                  </label>
                      ...
                      <div className="space-y-2">
                        ...
                        {currentPrediction.factors.map((factor, index) => (
                          <div key={index} className="flex justify-between text-sm">
                            ...
                            <span
                              className={factor.weight > 0 ? 'text-success' : 'text-destructive'}
                            >
                              {factor.weight > 0 ? '+' : ''}
          ...
          {activeTab === 'optimization' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label
                    htmlFor="optimization-min-margin"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    الحد الأدنى للهامش (%)
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      handleOptimizationParamsChange('minMargin', Number(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                  />
                </div>
                <div>
                  <label
                    htmlFor="optimization-max-margin"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    الحد الأقصى للهامش (%)
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      handleOptimizationParamsChange('maxMargin', Number(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                  />
                </div>
                <div>
                  <label
                    htmlFor="optimization-target-win-probability"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleOptimizationParamsChange('targetWinProbability', Number(e.target.value))
                    }
                    className="w-full px-3 py-2 border border-input rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label
                    htmlFor="optimization-risk-tolerance"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    تحمل المخاطر
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      handleOptimizationParamsChange(
                        'riskTolerance',
                ...
                <div>
                  <label
                    htmlFor="optimization-market-conditions"
                    className="block text-sm font-medium text-muted-foreground mb-2"
                  >
                    ظروف السوق
                  </label>
                ...
                <div className="bg-muted rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          <div
                            className={`text-xl font-bold ${
                              currentOptimization.riskLevel === 'low'
                                ? 'text-success'
                                : currentOptimization.riskLevel === 'medium'
                                  ? 'text-warning'
                                  : 'text-destructive'
                            }`}
                          >
                    ...
                    <div>
                      <h4 className="font-medium text-foreground mb-3">التوصيات:</h4>
                      <div className="space-y-3">
                        {currentOptimization.recommendations.map(
                          (rec: BidOptimization['recommendations'][number], index: number) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-success/10 rounded-lg p-4">
                      ...
                      <div className="text-sm text-success">
                        ...
                        {(priceSensitivity.riskAnalysis.lowRiskRange.max / 1000000).toFixed(1)}{' '}
                        مليون ريال
                      </div>
                    </div>
                    <div className="bg-warning/10 rounded-lg p-4">
                      ...
                      <div className="text-sm text-warning">
                        ...
                        {(priceSensitivity.riskAnalysis.mediumRiskRange.max / 1000000).toFixed(1)}{' '}
                        مليون ريال
                      </div>
                    </div>
                    <div className="bg-destructive/10 rounded-lg p-4">
                      ...
                      <div className="text-sm text-destructive">
                        ...
                        {(priceSensitivity.riskAnalysis.highRiskRange.max / 1000000).toFixed(1)}{' '}
                        مليون ريال
                      </div>
                    </div>
                  </div>
                        ...
                        <div>
                          <div
                            className={`text-3xl font-bold ${
                              currentTrendPrediction.direction === 'up'
                                ? 'text-success'
                                : currentTrendPrediction.direction === 'down'
                                  ? 'text-destructive'
                  ...
                  {scenarios.map((scenario) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground space-y-1">
                        <div>
                          القيمة: {(scenario.estimatedValue / 1000000).toFixed(1)} مليون ريال
                        </div>
