...
> = {
  ...
  decrease: {
    icon: '↘️',
    containerClass:
      'flex h-16 w-16 items-center justify-center rounded-full bg-destructive/10 text-destructive',
    textClass: 'text-destructive',
  },
  stable: {
    icon: '➡️',
    containerClass:
      'flex h-16 w-16 items-center justify-center rounded-full bg-muted/60 text-muted-foreground',
  ...
  ({
    ...
    const loadComparisonData = useCallback(async () => {
      ...
      try {
        ...
        const periodType =
          selectedPeriod === 'month-over-month'
            ? 'monthly'
            : selectedPeriod === 'quarter-over-quarter'
              ? 'quarterly'
              : 'yearly'
                ...
                <div className={`${MUTED_CARD_CLASS} p-4`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {comparison.insights.interpretation}
                      </span>
                    </div>
                    <div className="flex items-start gap-2">
              ...
              <div className={`${SUCCESS_CARD_CLASS} p-6`}>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                  <div className="text-center">
                    ...
                    <div
                      className={`mt-1 text-lg font-semibold ${TREND_DIRECTION_CLASSES[multiPeriodData.trend.direction]}`}
                    >
          ...
          {activeTab === 'annual' && annualReport && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                {annualReport.overview.map(
                  (overview: HistoricalComparisonResult, index: number) => {
                    ...
                    return (
                      <div key={index} className={`${CARD_BASE_CLASS} p-4`}>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {overview.comparison.direction === 'increase'
                            ? 'زيادة'
                            : overview.comparison.direction === 'decrease'
                              ? 'نقصان'
                              : 'استقرار'}
                        </div>
                      </div>
