...
export const AnalyticsFilters: React.FC<AnalyticsFiltersProps> = React.memo(
  ({
    ...
    const handleOutcomeChange = useCallback(
      (outcome: string, checked: boolean) => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>قم بتخصيص المرشحات لتحليل البيانات حسب احتياجاتك</CardDescription>
            </div>
        ...
        {isExpanded && (
          <CardContent className="space-y-6">
                  ...
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full justify-start text-left">
                      ...
                      {tempFilters.dateRange?.start
                        ? format(new Date(tempFilters.dateRange.start), 'dd/MM/yyyy', {
                            locale: ar,
                          })
                        : 'تاريخ البداية'}
                    </Button>
                ...
                <Popover open={endDateOpen} onOpenChange={setEndDateOpen}>
                  ...
                  <PopoverContent className="w-auto p-0">
                    <Calendar
                      mode="single"
                      selected={
                        tempFilters.dateRange?.end ? new Date(tempFilters.dateRange.end) : undefined
                      }
                      onSelect={(date) => {
                        if (date) {
                          updateFilter('dateRange', {
                ...
                {availableOutcomes.map((outcome) => (
                  <div key={outcome} className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleOutcomeChange(outcome, checked as boolean)
                      }
                    />
            ...
            <div className="flex items-center justify-end space-x-2 pt-4 border-t">
              ...
              <Button onClick={applyFilters} disabled={isApplying || !hasChanges}>
