...
export function PricingTemplateManager({
  ...
  const calculateValueSuitability = useCallback(
    (template: PricingTemplate, estimatedValue: number) => {
  ...
  const generateRecommendationReasons = useCallback(
    (template: PricingTemplate, context: typeof tenderContext) => {
  ...
  const analyzeTemplatePerformance = useCallback(
    (_performances: BidPerformance[], templates: PricingTemplate[]) => {
      return templates.map((template) => {
        ...
        if (tenderContext) {
          ...
          const templateSuitability = calculateValueSuitability(
            template,
            tenderContext.estimatedValue,
          )
  ...
  const generateTemplateInsights = useCallback(
    (
      context: typeof tenderContext,
      templatePerformance: { templateId: string; score: number; reasons: string[] }[],
    ) => {
      ...
      if (categoryTemplates.length > 0) {
        const avgAccuracy =
          categoryTemplates.reduce((sum, tp) => {
            const template = templates.find((t) => t.id === tp.templateId)
            return sum + (template?.averageAccuracy ?? 0)
          }, 0) / categoryTemplates.length

        insights.push(
          `متوسط دقة قوالب ${categoryLabels[context.category as keyof typeof categoryLabels]}: ${Math.round(avgAccuracy)}%`,
        )
      }
                ...
                <Card className="h-full hover:shadow-lg transition-all duration-300 group cursor-pointer">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      ...
                      >
                        <Star
                          className={`h-4 w-4 ${template.isStarred ? 'fill-warning text-warning' : 'text-muted-foreground'}`}
                        />
                      </Button>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="space-y-1">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <span
                            className={`text-sm font-medium ${getAccuracyColor(template.averageAccuracy)}`}
                          >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {template.costBreakdown.subcontractors}%
                          </span>
                        </div>
                      </div>
                    </div>
