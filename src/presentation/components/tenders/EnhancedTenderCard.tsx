...
const EnhancedTenderCard = memo<EnhancedTenderCardProps>(
  ({
        ...
        try {
          ...
          const recentOpportunities = marketOpportunities.filter((opportunity) => {
            const derivedCategory =
              (opportunity as { category?: string; segment?: string }).category ??
    ...
    const getCardClassName = () => {
      const baseClasses =
        'bg-card border shadow-sm hover:shadow-lg transition-all duration-300 group cursor-pointer'
          ...
          <CardContent className="p-5">
            ...
            <div className="flex items-start justify-between mb-4">
                ...
                {enablePredictiveAnalytics && predictiveData.loading ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span
                        className={`font-medium ${getWinChanceColor(predictiveData.winProbability)}`}
                      >
                    ...
                    <div className="flex items-center gap-4 text-xs">
                          ...
                          <AlertTriangle
                            className={`h-3 w-3 ${
                              predictiveData.riskLevel === 'high'
                                ? 'text-destructive'
                                : predictiveData.riskLevel === 'medium'
                                  ? 'text-warning'
                                  : 'text-success'
                            }`}
                          />
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-muted-foreground">
                          {predictiveData.competitorCount} منافسين
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex flex-col items-end gap-2 ml-4">
                ...
                />
                {completionInfo.isReadyToSubmit &&
                ...
                {completionInfo.isPricingCompleted &&
                  !completionInfo.isTechnicalFilesUploaded &&
                  !['submitted', 'won', 'lost'].includes(tender.status) && (
                    <StatusBadge
                      status="warning"
                  ...
                  {documentPrice > 0 && (
                    <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                      ...
                      <span className="font-medium text-card-foreground">
                        {formatCurrencyValue(documentPrice)}
                      </span>
                    </div>
                  )}
                </div>
