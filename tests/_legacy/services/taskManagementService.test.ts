...
describe('TaskManagementService', () => {
  ...
  describe('getTaskById', () => {
    ...
    it('should handle errors', async () => {
      vi.mocked(taskManagementService.getTaskById).mockRejectedValue(new Error('فشل في تحميل المهمة'))

      await expect(taskManagementService.getTaskById('task-1')).rejects.toThrow('فشل في تحميل المهمة')
    })
  })
  ...
  describe('createTask', () => {
    ...
    it('should validate task data', async () => {
      ...
      await expect(taskManagementService.createTask(invalidRequest)).rejects.toThrow('بيانات المهمة غير صحيحة')
    })
  ...
  describe('updateTaskProgress', () => {
    ...
    it('should validate progress range', async () => {
      ...
      await expect(taskManagementService.updateTaskProgress(invalidUpdate)).rejects.toThrow('نسبة التقدم يجب أن تكون بين 0 و 100')
    })
  })
