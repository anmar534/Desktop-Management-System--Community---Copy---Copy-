...
import {
  ...
  getStatusLabel,
  getStatusColorClass,
} from '@/presentation/components/projects/ProjectStatusBadge'
...
describe('ProjectStatusBadge', () => {
  describe('Rendering', () => {
    ...
    it('renders onHold status correctly', () => {
      ...
      expect(screen.getByTestId('status-badge-on-hold')).toBeInTheDocument()
    })
  ...
  describe('Icons', () => {
    ...
    it('displays correct icon for each status', () => {
      ...
      Object.entries(statuses).forEach(([status, icon]) => {
        const { container, unmount } = render(<ProjectStatusBadge status={status} showIcon />)
  ...
  describe('Helper Functions', () => {
    it('getStatusLabel returns correct Arabic label', () => {
      ...
      expect(getStatusLabel('onHold')).toBe('قيد الانتظار')
    ...
    it('getStatusColorClass returns correct color classes', () => {
      expect(getStatusColorClass('active')).toMatch(/text-success/)
      expect(getStatusColorClass('delayed')).toMatch(/text-error/)
      expect(getStatusColorClass('completed')).toMatch(/text-primary/)
    })
