...
import { ProjectFinancialSummary, getFinancialStatus } from '@/presentation/components/projects/ProjectFinancialSummary';
...
describe('ProjectFinancialSummary', () => {
  describe('Rendering - Full View', () => {
    ...
    it('displays budget correctly', () => {
      ...
      expect(screen.getByTestId('budget-value')).toHaveTextContent(/ألف/);
    });

    it('displays spent amount correctly', () => {
      render(<ProjectFinancialSummary budget={100000} spent={75000} />);
      expect(screen.getByTestId('spent-value')).toHaveTextContent(/75/);
      expect(screen.getByTestId('spent-value')).toHaveTextContent(/ألف/);
    });

    it('calculates and displays remaining amount', () => {
      render(<ProjectFinancialSummary budget={100000} spent={60000} />);
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/40/);
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/ألف/);
    });
  ...
  describe('Rendering - Compact View', () => {
    ...
    it('displays spent and budget in compact view', () => {
      const { container } = render(<ProjectFinancialSummary budget={100000} spent={60000} compact />);
      expect(container.textContent).toMatch(/60.*ألف/);
      expect(container.textContent).toMatch(/100.*ألف/);
    });
  ...
  describe('Financial Calculations', () => {
    it('calculates remaining correctly when under budget', () => {
      ...
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/30/);
    });

    it('calculates remaining correctly when over budget', () => {
      render(<ProjectFinancialSummary budget={100000} spent={120000} />);
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/20/);
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/تجاوز/);
    });
    ...
    it('handles zero spent gracefully', () => {
      ...
      expect(screen.getByTestId('spent-value')).toHaveTextContent('0');
      expect(screen.getByTestId('remaining-value')).toHaveTextContent(/100/);
    });
  ...
  describe('Currency Formatting', () => {
    it('uses default Saudi Riyal currency', () => {
      render(<ProjectFinancialSummary budget={100000} spent={50000} />);
      expect(screen.getByTestId('budget-value')).toHaveTextContent(/ر\.س\./);
    });
  ...
  describe('Styling', () => {
    ...
    it('applies custom className to compact view', () => {
      render(<ProjectFinancialSummary budget={100000} spent={50000} compact className="custom-compact" />);
  ...
  describe('Edge Cases', () => {
    it('handles exact budget match', () => {
      ...
      expect(screen.getByTestId('remaining-value')).toHaveTextContent('0');
    });
