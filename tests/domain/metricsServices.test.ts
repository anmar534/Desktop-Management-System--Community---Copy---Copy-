...
import type { CashflowEntry, ProjectCostSnapshot } from '@/domain/contracts/metrics'
...
describe('ProjectCostAnalyzer', () => {
  ...
  it('normalizes invalid numbers and falls back to default category', () => {
    ...
    expect(
      summary.items.onTrackCount + summary.items.overBudgetCount + summary.items.underBudgetCount,
    ).toBe(2)
  })
})
