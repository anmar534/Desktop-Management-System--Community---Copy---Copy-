...
function chooseCanonical(entry: RawBoqItem): { canonical: string; finalDescription: string } {
  ...
  if (entry.description && !isPlaceholder(entry.description))
    candidates.push(String(entry.description))
  const canonical =
    candidates.find((candidate) => candidate && candidate.length > 0) ??
    (typeof entry.description === 'string' ? entry.description : null) ??
    'بدون وصف'
  const finalDescription = isPlaceholder(entry.description)
    ? canonical
    : String(entry.description ?? canonical)
  return { canonical, finalDescription }
}

export function runBackfillCanonicalDescriptions(): {
  updatedBOQs: number
  updatedItems: number
  totalBOQs: number
  totalItems: number
  ...
  if (updatedBOQs > 0) {
    ...
    console.log('[Backfill] Saved', {
      updatedBOQs,
      updatedItems,
      totalBOQs: boqArray.length,
      totalItems,
    })
  } else {
