...
export const TenderPricingProcess: React.FC<TenderPricingProcessProps> = ({ 
  ...
  const handleTemplateSave = useCallback((templateData: any) => {
    try {
      ...
      toast.success(`╪ز┘à ╪ص┘╪╕ ╪د┘┘é╪د┘╪ذ "${template.name}" ╪ذ┘╪ش╪د╪ص`);
      return template;
    } catch (error) {
  ...
  const notifyPricingUpdate = useCallback(() => {
    try {
        ...
        try {
          ...
          const rawEntries: [string, PricingItemInput][] = Array.from(pricingData.entries()).map(([id, data]) => {
            ...
            const entry: PricingItemInput = {
              ...
              description: sourceItem?.description ?? `╪د┘╪ذ┘╪» ${id}`,
              unit: sourceItem?.unit ?? '┘ê╪ص╪»╪ر',
          ...
          console.warn('ظأبي╕ Engine authoring enrichment failed, falling back to legacy only', e);
          engineMap = null;
        }
      }
      ...
      // ╪د╪│╪ز╪«╪»╪د┘à ╪«╪»┘à╪ر ╪د┘┘à╪▓╪د┘à┘╪ر ╪د┘╪ش╪»┘è╪»╪ر ╪ذ╪»┘╪د┘ï ┘à┘ ╪د┘╪ص╪»╪س ╪د┘┘à╪ذ╪د╪┤╪▒
      ...
      // ╪ز╪┤╪║┘è┘ ╪«╪»┘à╪ر ╪د┘┘à╪▓╪د┘à┘╪ر ┘╪╢┘à╪د┘ ╪ز╪ص╪»┘è╪س ╪د┘┘╪╕╪د┘à ╪د┘┘à╪▒┘â╪▓┘è
      pricingDataSyncService.forceSyncTender(tender.id).then(success => {
        if (success) {
          console.log('ظ£à [TenderPricingProcess] ╪ز┘à╪ز ┘à╪▓╪د┘à┘╪ر ╪د┘╪ذ┘è╪د┘╪د╪ز ┘à╪╣ ╪د┘┘╪╕╪د┘à ╪د┘┘à╪▒┘â╪▓┘è')
        }
      }).catch(error => {
        console.warn('ظأبي╕ [TenderPricingProcess] ┘╪┤┘ ┘┘è ┘à╪▓╪د┘à┘╪ر ╪د┘╪ذ┘è╪د┘╪د╪ز:', error)
      });
      
      console.log('≡ا¤¤ [TenderPricingProcess] ╪ز┘à ╪ح╪▒╪│╪د┘ ╪ص╪»╪س ╪ز╪ص╪»┘è╪س ╪د┘╪ذ┘è╪د┘╪د╪ز:', {
        tenderId: tender.id,
        items: quantityTableWithPricing.length,
        engineAuthoring: PRICING_FLAGS.USE_ENGINE_AUTHORING
      });

      // (Legacy Dual-Write Removed 2025-09): ╪ص╪░┘ ┘à╪│╪د╪▒ dualWritePricing.
    } catch (error) {
      console.error('ظإî [TenderPricingProcess] ╪«╪╖╪ث ┘┘è ╪ح╪▒╪│╪د┘ ╪ح╪┤╪╣╪د╪▒ ╪د┘╪ز╪ص╪»┘è╪س:', error);
    }
  }, [
    defaultPercentages,
    domainPricing.enabled,
  ...
  const persistPricingAndBOQ = useCallback(async (map: Map<string, PricingData>) => {
    ...
    const isMeaningfulDescription = (value: string): boolean =>
      value.length > 0 && !/^╪د┘╪ذ┘╪»\s*\d+$/i.test(value) && !/^╪ذ┘╪»\s*\d+$/i.test(value) && !/╪║┘è╪▒\s*┘à╪ص╪»╪»/.test(value);
      ...
      const items = quantityItems
        .map<PersistedBOQItem | null>((quantityItem) => {
          ...
          Object.entries(quantityItem).forEach(([key, value]) => {
            ...
            if (/desc|description|┘ê╪╡┘/i.test(key)) {
          ...
          const fallback = quantityItem.itemNumber ? `╪د┘╪ذ┘╪» ${quantityItem.itemNumber}` : `╪د┘╪ذ┘╪» ${quantityItem.id}`;
  ...
  // ╪ص╪│╪د╪ذ ╪╢╪▒┘è╪ذ╪ر ╪د┘┘é┘è┘à╪ر ╪د┘┘à╪╢╪د┘╪ر 15%
  const calculateVAT = useCallback(() => {
    return calculateItemsTotal() * 0.15;
  }, [calculateItemsTotal]);

  // ╪ص╪│╪د╪ذ ╪ح╪ش┘à╪د┘┘è ┘é┘è┘à╪ر ╪د┘┘à╪┤╪▒┘ê╪╣ ┘à╪╣ ╪╢╪▒┘è╪ذ╪ر ╪د┘┘é┘è┘à╪ر ╪د┘┘à╪╢╪د┘╪ر
  ...
  const updateTenderStatus = useCallback(() => {
    ...
    updateTender(tender.id, persistencePayload).catch(error => {
      console.error('╪ز╪╣╪░╪▒ ╪ز╪ص╪»┘è╪س ╪ذ┘è╪د┘╪د╪ز ╪د┘┘à┘╪د┘╪│╪ر ╪ذ╪╣╪» ┘à╪▓╪د┘à┘╪ر ╪د┘╪ز╪│╪╣┘è╪▒', error);
    });
    ...
    console.log('≡ا¤ ╪ز┘à ╪ز╪ص╪»┘è╪س ╪ص╪د┘╪ر ╪د┘┘à┘╪د┘╪│╪ر:', {
      tenderId: tender.id,
      status,
      pricingStatus,
    ...
    if (completionPercentage === 100) {
      toast.success('╪ز┘à ╪ح┘â┘à╪د┘ ╪د┘╪ز╪│╪╣┘è╪▒', {
        description: '╪ز┘à ╪ح┘â┘à╪د┘ ╪ز╪│╪╣┘è╪▒ ╪ش┘à┘è╪╣ ╪ذ┘┘ê╪» ╪د┘┘à┘╪د┘╪│╪ر ╪ذ┘╪ش╪د╪ص',
        duration: 5000
      });
    }
